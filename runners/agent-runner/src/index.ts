 ...
 * Runs as a host process, receives config via stdin, outputs result to stdout
 *
 * Input protocol:
...
const GLOBAL_DIR =
  ...
  path.join(process.cwd(), '..', '..', 'groups', 'global');
const IPC_DIR =
  process.env.NANOCLAW_IPC_DIR ||
  path.join(process.cwd(), '..', '..', 'data', 'ipc', 'default');
...
async function runQuery(
  ...
  const requestedModel = sdkEnv.CLAUDE_MODEL;
