...
トリガーワード（デフォルト：`@Dobby`）でアシスタントに話しかけます：

```
@Dobby 毎朝9時に営業パイプラインの概要を送って（Obsidian vaultフォルダにアクセス可能）
@Dobby 毎週金曜に過去1週間のgit履歴をレビューして、差異があればREADMEを更新して
@Dobby 毎週月曜の朝8時に、Hacker NewsとTechCrunchからAI関連のニュースをまとめてブリーフィングを送って
```

所有または管理しているチャネルからは、グループやタスクを管理できます：
```
@Dobby 全グループのスケジュールタスクを一覧表示して
@Dobby 月曜のブリーフィングタスクを一時停止して
@Dobby Family Chatグループに参加して
```
