...
You can force a specific model by specifying it in the lookup command:

```bash
# Force Sonar Pro Search for fast lookup
./scripts/lookup.sh "your query" "perplexity/sonar-pro"

# Force Sonar Reasoning Pro for deep analysis
./scripts/lookup.sh "your query" "perplexity/sonar-reasoning-pro"

# Default (Sonar Pro Search)
./scripts/lookup.sh "your query"
```

Alternatively, use the Claude SDK's WebSearch tool for quick lookups, or curl directly against the OpenRouter API.

## Technical Integration

### How to Perform Lookups

This skill can be used in three ways within the container:

**1. Shell script (recommended for programmatic use):**
```bash
# Simple lookup
./scripts/lookup.sh "Recent advances in CRISPR gene editing 2024"

# With specific model
./scripts/lookup.sh "Compare mRNA vs traditional vaccines" "perplexity/sonar-reasoning-pro"
```

**2. curl directly against OpenRouter API:**
```bash
...
**3. WebSearch/WebFetch tools (for quick metadata and non-academic lookups):**
Use the Claude SDK's built-in WebSearch and WebFetch tools for quick metadata verification, DOI lookups, and non-academic sources.
...
**API Requirements**:
- OpenRouter API key (available as `OPENROUTER_API_KEY` environment variable in the container)
...
**Pro Tip**: The automatic selection is optimized for most use cases. Only use manual override if you have specific requirements or know the query needs deeper reasoning than detected.
...
- Fall back to WebSearch/WebFetch tools for basic lookups
...
In addition to research-lookup, the agent has access to **WebSearch** and **WebFetch** for:
...
| Task | Tool |
|------|------|
| Find academic papers | research-lookup (via lookup.sh or curl) |
