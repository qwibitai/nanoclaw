...
Each directory is an installed skill. The directory name is the skill name (e.g., `browser-use` → `/browser-use`).
...
```bash
which browser-agent 2>/dev/null && echo "browser-agent: available" || echo "browser-agent: not found"
```
