...
function readSessionAwareness(ownContainerId?: string): string {
  ...
  try {
    ...
    log(
      `Failed to read session awareness: ${err instanceof Error ? err.message : String(err)}`,
    );
    return '';
  }
}
...
async function main(): Promise<void> {
  ...
  // Merge secrets passed via stdin into the environment for the Claude SDK.
  // When OneCLI proxy is active, credentials flow through the proxy instead.
  // When OneCLI is not configured, secrets are passed directly via stdin.
  ...
  if (awarenessContext) {
    ...
    log(
      `Injected session awareness (${sessionCount} other active session${sessionCount !== 1 ? 's' : ''})`,
    );
  }
