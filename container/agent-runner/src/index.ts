...
<decisions>[重要决策和结论]</decisions>
  ...
  const systemAppend = [claudeMd + claudeMdCompressInstruction, globalClaudeMd, skillPrompts, constraintInstruction]
    .filter(Boolean).join('\n\n---\n\n') || undefined;

  // 机制一补丁：compactInstruction 注入到 prompt 末尾（resume 模式下 systemPrompt.append 不生效）
  if (compactInstruction) {
    effectivePrompt = `${effectivePrompt}\n\n${compactInstruction}`;
    log(`[token-opt] M1: compactInstruction injected into prompt (resume-mode workaround)`);
  }
  ...
  for await (const message of query({
    ...
    if (message.type === 'system' && message.subtype === 'init') {
      ...
      capturedModel = (message as { model?: string }).model ?? process.env.ANTHROPIC_MODEL ?? null;
