...
function loadMcpConfig(filePath) {
    ...
    try {
        ...
        log(`Failed to load MCP config from ${filePath}: ${err instanceof Error ? err.message : String(err)}`);
        return {};
    }
}
...
async function runQuery(prompt, sessionId, mcpServerPath, containerInput, sdkEnv, resumeAt) {
    ...
    if (Object.keys(additionalMcpServers).length > 0) {
        log(`Additional MCP servers: ${Object.keys(additionalMcpServers).join(', ')}`);
    }
