...
async function spawnOpencodeServer(config: Record<string, unknown>, timeoutMs = 10_000): Promise<{ url: string; proc: ChildProcess }> {
  const xdgDataHome = process.env.XDG_DATA_HOME || '/opencode-xdg';
  const opencodeDir = `${xdgDataHome}/opencode`;
  if (!fs.existsSync(opencodeDir)) {
    fs.mkdirSync(opencodeDir, { recursive: true });
  }
  fs.writeFileSync(`${opencodeDir}/config.json`, JSON.stringify(config, null, 2));
...
function buildOpenCodeConfig(options: ProviderOptions): Record<string, unknown> {
  ...
  const proxyUrl = process.env.ANTHROPIC_BASE_URL || 'https://opencode.ai/zen/go/v1';
