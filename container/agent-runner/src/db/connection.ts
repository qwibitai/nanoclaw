...
export function getInboundDb(): Database {
  if (!_inbound) {
    _inbound = new Database(_inboundPath, { readonly: true });
...
/** For tests — creates temporary DB files with the session schemas. */
export function initTestSessionDb(): { inbound: Database; outbound: Database } {
  closeSessionDb();

  _testDir = fs.mkdtempSync(path.join(os.tmpdir(), 'nanoclaw-agent-test-'));
  _inboundPath = path.join(_testDir, 'inbound.db');
  _outboundPath = path.join(_testDir, 'outbound.db');
  _heartbeatPath = path.join(_testDir, '.heartbeat');
