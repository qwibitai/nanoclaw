...
server.tool(
  ...
  "Send a message to the user or group immediately while you're still running. Use this for progress updates or to send multiple messages. You can call this multiple times.",
  {
    ...
    file_path: z.string().optional().describe('Absolute path to a file inside the container (must start with /workspace/) to send as a media attachment.'),
...
server.tool(
  'send_reaction',
  'React to a message with an emoji. Use this to acknowledge a message with a thumbs up (👍) when you anticipate your response will take more than 10 seconds, so the user knows you received it.',
  {
    message_id: z.string().describe('The message ID to react to (from the incoming message)'),
    emoji: z.string().default('👍').describe('The emoji to react with (default: 👍)'),
    target_author: z.string().optional().describe('The author of the message to react to (phone number or UUID). Required for Signal group reactions.'),
  },
  async (args) => {
    const data: Record<string, string | undefined> = {
      type: 'reaction',
      ...
      emoji: args.emoji || '👍',
      targetAuthor: args.target_author || undefined,
      groupFolder,
      timestamp: new Date().toISOString(),
    };
...
server.tool(
  'send_image',
  'Send an image or file to the user or group. The file must exist in the container filesystem (e.g., /workspace/group/image.png). You can include an optional caption.',
  {
    file_path: z.string().describe('Absolute path to the image file inside the container (e.g., /workspace/group/chart.png)'),
  ...
  async (args) => {
    ...
    return { content: [{ type: 'text' as const, text: `Image sent: ${args.file_path}` }] };
  },
);
