...
Then run `/setup`. Claude Code handles everything: dependencies, authentication (Discord Token), container setup, service configuration.
...
- **Discord I/O** - Message Claude from your Discord server or DM
- **Isolated group context** - Each channel/DM has its own `CLAUDE.md` memory, isolated filesystem, and runs in its own container sandbox with only that filesystem mounted
- **Main channel** - Your private channel (self-chat or admin channel) for admin control; every other group is completely isolated
...
- `/add-slack` - Add Slack
- `/add-whatsapp` - Re-add WhatsApp support (migrated away from it)
