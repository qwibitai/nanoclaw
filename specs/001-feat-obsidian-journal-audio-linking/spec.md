...
### User Story 4 - QMD-Powered Inline Note Linking (Priority: P2)

When a note is created or appended, the system uses QMD to search the vault for related existing notes and links them inline within the transcribed content using Obsidian wikilinks. Only notes that actually exist in the vault are linked. If no relevant notes are found, no links are added.
...
2. **Given** QMD returns search results, **When** the agent creates the note, **Then** only notes that actually exist as files in the vault are linked — no broken links.
...
4. **Given** QMD is unavailable or returns an error, **When** a note is created, **Then** the note is still created successfully without links — the system degrades gracefully.
...
- What happens when QMD returns notes that have been deleted since the last index update? The system must verify file existence before linking.
...
- **FR-007**: System MUST use QMD to search for related notes in the vault when creating or appending notes.
- **FR-008**: System MUST verify that each note returned by QMD actually exists as a file in the vault before creating a wikilink.
- **FR-009**: System MUST NOT add wikilinks when QMD finds no relevant notes or when QMD is unavailable.
...
- **SC-005**: Note creation succeeds even when QMD or audio persistence fails, with the available components still present.
...
- QMD is already installed and indexed on the host system; if it is not, the system degrades gracefully (as it does today).
