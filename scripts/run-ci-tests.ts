...
async function runMatrixEntry(
  ...
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'codeclaw-ci-'));
  ...
  try {
    ...
    // Initialize codeclaw dir
    execSync('npx tsx -e "import { initCodeclawDir } from \'./skills-engine/index.js\'; initCodeclawDir();"', {
      cwd: tmpDir,
      stdio: 'pipe',
      timeout: 30_000,
    });
