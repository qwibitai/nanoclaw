...
export interface WhatsAppProviderOpts {
  onMessage: OnInboundMessage;
  onChatMetadata: OnChatMetadata;
  registeredGroups: () => Record<string, RegisteredGroup>;
}

export class WhatsAppProvider implements Channel {
  name = 'whatsapp';
  ...
  private queueFlushRetryTimer: ReturnType<typeof setTimeout> | null = null;
  private manualDisconnect = false;
  private pairingCodeRequested = false;

  private opts: WhatsAppProviderOpts;

  constructor(opts: WhatsAppProviderOpts) {
    this.opts = opts;
  }
  ...
  private async connectInternal(onFirstOpen?: () => void): Promise<void> {
    ...
    this.sock.ev.on('connection.update', (update) => {
      ...
      if (connection === 'close') {
        ...
        this.clearQueueFlushRetryTimer();
    ...
    try {
      ...
      while (this.outgoingQueue.length > 0) {
        ...
        try {
          ...
          logger.warn(
            { err, jid: item.jid, queueSize: this.outgoingQueue.length },
            'Failed to flush queued message, will retry',
          );
          this.scheduleQueueFlushRetry();
          break;
        }
  ...
  private scheduleQueueFlushRetry(): void {
    if (this.manualDisconnect || this.queueFlushRetryTimer) return;

    this.queueFlushRetryTimer = setTimeout(() => {
      this.queueFlushRetryTimer = null;
      ...
      this.flushOutgoingQueue().catch((err) => {
        logger.error({ err }, 'Queue flush retry failed');
        this.scheduleQueueFlushRetry();
      });
    }, 1000);
  }

  private clearQueueFlushRetryTimer(): void {
    if (this.queueFlushRetryTimer) {
      clearTimeout(this.queueFlushRetryTimer);
      this.queueFlushRetryTimer = null;
    }
  }
