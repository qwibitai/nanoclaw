...
export function initChatDatabase(dataDir: string): void {
  ...
  // Normalize any TEXT-typed `created_at` values (legacy imports or external
  // inserters that passed ISO strings instead of epoch-ms). SQLite sorts mixed
  // INTEGER/TEXT with text above all integers, which pushes real timestamps
  // out of `ORDER BY created_at DESC LIMIT N` windows.
  const normalized = db
    .prepare(
      `UPDATE chat_messages
       SET created_at = CAST(ROUND((julianday(created_at) - 2440587.5) * 86400000) AS INTEGER)
       WHERE typeof(created_at) = 'text'`,
    )
    .run();
  if (normalized.changes > 0) {
    logger.warn(
      { rows: normalized.changes },
      'chat_messages: normalized TEXT created_at values to INTEGER ms',
    );
  }
  ...
  db.exec(`
    ...
    CREATE INDEX IF NOT EXISTS idx_push_subscriptions_identity
      ON push_subscriptions(identity);
  `);
...
export function getChatMessagesAfterId(
  roomId: string,
  afterId: string,
  limit = 200,
): ChatMessage[] {
  ...
  if (!ref) {
    logger.warn(
      { roomId, afterId },
      'getChatMessagesAfterId: reference message not found',
    );
    return [];
  }
  const rows = getDb()
    .prepare(
      'SELECT * FROM chat_messages WHERE room_id = ? AND created_at > ? ORDER BY created_at ASC LIMIT ?',
    )
    .all(roomId, ref.created_at, limit) as Array<Record<string, unknown>>;
...
export function deleteChatMessage(id: string, sender: string): boolean {
  const result = getDb()
    .prepare('DELETE FROM chat_messages WHERE id = ? AND sender = ?')
    .run(id, sender);
...
export function deletePushSubscriptionForIdentity(
  endpoint: string,
  identity: string,
): void {
  getDb()
    .prepare(
      'DELETE FROM push_subscriptions WHERE endpoint = ? AND identity = ?',
    )
    .run(endpoint, identity);
}

export function getPushSubscriptionOwner(endpoint: string): string | null {
  const row = getDb()
    .prepare('SELECT identity FROM push_subscriptions WHERE endpoint = ?')
    .get(endpoint) as { identity: string } | undefined;
  return row ? row.identity : null;
}

export function getPushSubscriptionsExcludingIdentity(
  identity: string,
): PushSubscriptionRow[] {
  return getDb()
    .prepare(
      'SELECT endpoint, identity, keys_json FROM push_subscriptions WHERE identity != ?',
    )
    .all(identity) as PushSubscriptionRow[];
}
