...
describe('pruneSteerIdempotency — D7', () => {
  beforeEach(() => {
    ...
    getDb().prepare("INSERT OR IGNORE INTO users (id, kind, display_name, created_at) VALUES ('u1', 'test', NULL, datetime('now'))").run();
  });
  ...
  it('test_prune_removes_old_applied', () => {
    ...
    const rows = getDb().prepare("SELECT idempotency_key FROM steer_idempotency WHERE status = 'applied'").all() as { idempotency_key: string }[];
  ...
  it('test_prune_preserves_recent_pending', () => {
    ...
    const rows = getDb().prepare("SELECT idempotency_key FROM steer_idempotency WHERE idempotency_key = 'pend-new'").all();
