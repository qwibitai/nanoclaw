...
function createSchema(database: Database.Database): void {
  ...
  // Add dev group columns if they don't exist (migration for existing DBs)
  try {
    database.exec(
      `ALTER TABLE registered_groups ADD COLUMN is_dev INTEGER DEFAULT 0`,
    );
  } catch {
    /* column already exists */
  }
  try {
    database.exec(
      `ALTER TABLE registered_groups ADD COLUMN feature_branch TEXT`,
    );
  } catch {
    /* column already exists */
  }
...
export function setRegisteredGroup(
  ...
  db.prepare(
    `INSERT OR REPLACE INTO registered_groups (jid, name, folder, trigger_pattern, added_at, container_config, requires_trigger, is_dev, feature_branch)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
  ).run(
    jid,
    group.name,
