...
function createSchema(database: Database.Database): void {
  ...
  // Add bot column if it doesn't exist (migration for multi-bot support)
  try {
    database.exec(`ALTER TABLE registered_groups ADD COLUMN bot TEXT`);
  } catch {
    /* column already exists */
  }
...
export function setRegisteredGroup(jid: string, group: RegisteredGroup): void {
  ...
  db.prepare(
    `INSERT OR REPLACE INTO registered_groups (jid, name, folder, trigger_pattern, added_at, container_config, requires_trigger, is_main, bot)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
  ).run(
    jid,
    group.name,
