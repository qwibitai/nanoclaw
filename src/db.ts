...
function createSchema(database: Database.Database): void {
  ...
  // Add attachments column (JSON) for image/file metadata (vision support)
  try {
    database.exec(
...
export function getNewMessages(
  ...
  const rows = db
    .prepare(sql)
    .all(lastTimestamp, ...jids, `${botPrefix}:%`) as (NewMessage & { attachments?: string | null })[];
  for (const row of rows) {
    if (typeof row.attachments === 'string') {
      try { row.attachments = JSON.parse(row.attachments); } catch { row.attachments = undefined; }
    } else {
...
export function getMessagesSince(
  ...
  const rows = db
    .prepare(sql)
    .all(chatJid, sinceTimestamp, `${botPrefix}:%`) as (NewMessage & { attachments?: string | null })[];
  for (const row of rows) {
    if (typeof row.attachments === 'string') {
      try { row.attachments = JSON.parse(row.attachments); } catch { row.attachments = undefined; }
    } else {
