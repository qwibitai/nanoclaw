...
export interface InternetAccessPolicy {
  ...
  /**
   * If set, this agent can drive a host-side Chrome via CDP at
   * host.docker.internal:<cdpPort>. The provider emits per-agent ACLs that
   * bypass the global Safe_ports deny (since CDP ports aren't in 80/443) and
   * bypass the OneCLI parent peer (OneCLI can't tunnel to host.docker.internal
   * — that's a Squid-bridge-network destination, not an internet endpoint).
   * Each agent should use a unique port so the per-(src, dst, port) ACL grants
   * are mutually exclusive: agent A cannot reach agent B's CDP port.
   */
  cdpPort?: number;
}
...
function generateSocatForwardsConfig(agents: AgentGroup[], ips: IpMap): string {
  ...
  for (const agent of agents) {
    ...
    lines.push(`${policy.cdpPort} ${ip}/32 host.docker.internal ${policy.cdpPort}`);
  }
  lines.push('');
  return lines.join('\n');
}
...
function reconfigureSquid(): void {
  if (!containerRunning(CONTAINER_NAME)) return;
  try {
    execFileSync(CONTAINER_RUNTIME_BIN, ['exec', CONTAINER_NAME, 'squid', '-k', 'reconfigure'], { stdio: 'pipe' });
