...
export async function applyDispatchComplete(content: Record<string, unknown>, callerSession: Session): Promise<void> {
  ...
  if (!taskId) {
    log.warn('applyDispatchComplete: missing task_id', { sessionId: callerSession.id });
    return;
  }

  const task = getTaskById(taskId);
  if (!task) {
    log.warn('applyDispatchComplete: task not found', { taskId });
    return;
  }

  // Two-column auth: task_id + child_session_id must match caller
  if (task.child_session_id !== callerSession.id) {
    log.warn('applyDispatchComplete: auth failed — child_session_id mismatch', {
      taskId,
  ...
  if (!transitioned) {
    log.debug('applyDispatchComplete: task already in terminal state, skipping notify', { taskId });
    return;
  }
  ...
  try {
    ...
    void wakeContainer(parentSession).catch((err) =>
      log.warn('applyDispatchComplete: wakeContainer(parent) failed', { taskId, err }),
    );
  } catch (err) {
    log.warn('applyDispatchComplete: failed to notify parent', { taskId, err });
  }
}
...
export async function applyDispatchFailed(content: Record<string, unknown>, callerSession: Session): Promise<void> {
  ...
  if (!taskId) {
    log.warn('applyDispatchFailed: missing task_id', { sessionId: callerSession.id });
    return;
  }

  const task = getTaskById(taskId);
  if (!task) {
    log.warn('applyDispatchFailed: task not found', { taskId });
    return;
  }

  // Two-column auth: task_id + child_session_id must match caller
  if (task.child_session_id !== callerSession.id) {
    log.warn('applyDispatchFailed: auth failed — child_session_id mismatch', {
      taskId,
      expected: task.child_session_id,
      got: callerSession.id,
    });
  ...
  if (!transitioned) {
    log.debug('applyDispatchFailed: task already in terminal state, skipping notify', { taskId });
    return;
  }
  ...
  try {
    ...
    void wakeContainer(parentSession).catch((err) =>
      log.warn('applyDispatchFailed: wakeContainer(parent) failed', { taskId, err }),
    );
  } catch (err) {
    log.warn('applyDispatchFailed: failed to notify parent', { taskId, err });
  }
}
