 ...
 *
 * This prevents transient API unavailability from permanently blocking tasks
 * by giving the system time to recover between retry attempts.
 */
const dispatchSkipTicks = new Map<string, number>();
...
async function dispatchReadyTasks(deps: SchedulerDependencies): Promise<void> {
  ...
  try {
    ...
    for (const task of tasks) {
      ...
      const dispatched = await dispatchTask(task, target.jid, target.folder, deps, log);
