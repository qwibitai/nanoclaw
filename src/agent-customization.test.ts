...
describe('resolveSerializableAgentSettings', () => {
  it('defaults instructions and skillsSources to null', () => {
...
describe('AgentRegistryDb instructions/skills', () => {
  it('round-trips instructions and skillsSources', () => {
    const db = initAgentRegistryDb(tmpDir);
    try {
      const settings = resolveSerializableAgentSettings(
        'bot',
        { instructions: 'Be concise.', skills: ['/a/skill-a', '/a/skill-b'] },
        tmpDir,
      );
  ...
  it('migrates existing database without new columns', async () => {
    ...
    `);
    raw.prepare('INSERT INTO agents VALUES (?, ?, ?, ?, ?, ?, ?)').run(
      'old', 'old00001', '/tmp/old', 'Andy', null, '2024-01-01', '2024-01-01',
    );
    raw.close();
...
describe('syncAgentCustomizations', () => {
  ...
  it('copies skill directory with supporting files', () => {
    ...
    expect(fs.readFileSync(path.join(copied, 'SKILL.md'), 'utf-8')).toContain('name: echo');
    expect(fs.readFileSync(path.join(copied, 'helper.ts'), 'utf-8')).toContain('helper for echo');
  });
  ...
  it('copies multiple skills', () => {
    ...
    expect(fs.existsSync(path.join(agentDir, 'skills', 'alpha', 'SKILL.md'))).toBe(true);
    expect(fs.existsSync(path.join(agentDir, 'skills', 'beta', 'SKILL.md'))).toBe(true);
  });
  ...
  it('two agents get independent agentDirs', () => {
    ...
    expect(fs.existsSync(path.join(agentA, 'skills', 'shared-name'))).toBe(true);
