...
describe('TRIGGER_PATTERN', () => {
  it('matches @Flux at start of message', () => {
    expect(TRIGGER_PATTERN.test('@Flux hello')).toBe(true);
  });
  ...
  it('does not match when not at start of message', () => {
    expect(TRIGGER_PATTERN.test('hello @Flux')).toBe(false);
  });

  it('does not match partial name like @Fluxion (word boundary)', () => {
    expect(TRIGGER_PATTERN.test('@Fluxion hello')).toBe(false);
  });

  it('matches with word boundary before apostrophe', () => {
    expect(TRIGGER_PATTERN.test("@Flux's thing")).toBe(true);
  });

  it('matches @Flux alone (end of string is a word boundary)', () => {
    expect(TRIGGER_PATTERN.test('@Flux')).toBe(true);
  });

  it('matches with leading whitespace after trim', () => {
    // The actual usage trims before testing: TRIGGER_PATTERN.test(m.content.trim())
    expect(TRIGGER_PATTERN.test('@Flux hey'.trim())).toBe(true);
  });
});
...
describe('trigger gating (requiresTrigger interaction)', () => {
  ...
  it('non-main group with requiresTrigger=true processes when trigger present', () => {
    const msgs = [makeMsg({ content: '@Flux do something' })];
