 ...
 * No Docker required — agents run directly on the host process.
 */
...
/**
 * Resolve the path to the compiled agent-runner entry point.
 * Looks for it relative to the project root.
 */
function resolveAgentRunnerEntry(): string {
  return path.join(process.cwd(), 'container', 'agent-runner', 'dist', 'index.js');
}

/**
 * Set up the per-group .claude/ sessions directory, write settings.json,
 * and sync skills from container/skills/ into the group's .claude/skills/.
 */
function setupGroupSessionsDir(group: RegisteredGroup): string {
  const groupSessionsParent = path.join(DATA_DIR, 'sessions', group.folder);
  const groupSessionsDir = path.join(groupSessionsParent, '.claude');
  ...
  // HOME is set to groupSessionsParent so claude-code finds .claude/ at $HOME/.claude
  return groupSessionsParent;
}
...
export async function runContainerAgent(
  ...
  // HOME dir for .claude/ session isolation
  const homeDir = setupGroupSessionsDir(group);
  ...
  const agentRunnerEntry = resolveAgentRunnerEntry();

  const globalDir = path.join(GROUPS_DIR, 'global');

  // Build child process environment: inherit host env, override paths for isolation
  ...
  return new Promise((resolve) => {
    ...
    const killOnTimeout = () => {
      ...
      // Force kill after 10 seconds if graceful stop fails
      setTimeout(() => {
        if (!container.killed) {
          logger.warn({ group: group.name, processName }, 'Force killing agent after SIGTERM timeout');
