...
function buildVolumeMounts(
  ...
  if (isMain) {
    ...
    // Note: Docker installs shadow .env here via a file bind mount to prevent
    // the agent from reading host secrets. Apple Container only supports directory
    // mounts, so the shadow is skipped. Credentials are injected by the credential
    // proxy and never need to be read from .env inside the container.
  ...
  if (fs.existsSync(agentRunnerSrc)) {
    const newestSrcMtime = (dir: string): number => {
      let max = 0;
      for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
        const full = path.join(dir, entry.name);
        if (entry.isDirectory()) {
          max = Math.max(max, newestSrcMtime(full));
        } else {
    ...
    const needsCopy =
      ...
      newestSrcMtime(agentRunnerSrc) > fs.statSync(cachedIndex).mtimeMs;
    if (needsCopy) {
...
function buildContainerArgs(
  ...
  // Route API calls through the credential proxy so containers never see real secrets
  args.push(
    '-e',
    `ANTHROPIC_BASE_URL=http://${CONTAINER_HOST_GATEWAY}:${CREDENTIAL_PROXY_PORT}`,
  );
  args.push('-e', 'ANTHROPIC_API_KEY=proxy-injected');
