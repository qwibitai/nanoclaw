...
function readSecrets(): Record<string, string> {
  return readEnvFile(['CLAUDE_CODE_OAUTH_TOKEN', 'ANTHROPIC_API_KEY', 'PARCEL_API_KEY', 'TOMORROW_IO_API_KEY', 'GEMINI_API_KEY']);
}
...
function buildContainerArgs(
  ...
  // Pass API keys so curl commands in Bash can access them
  const envVars = readEnvFile(['PARCEL_API_KEY', 'TOMORROW_IO_API_KEY', 'GEMINI_API_KEY']);
  if (envVars.PARCEL_API_KEY) {
    args.push('-e', `PARCEL_API_KEY=${envVars.PARCEL_API_KEY}`);
  }
  if (envVars.TOMORROW_IO_API_KEY) {
    args.push('-e', `TOMORROW_IO_API_KEY=${envVars.TOMORROW_IO_API_KEY}`);
  }
  if (envVars.GEMINI_API_KEY) {
    args.push('-e', `GEMINI_API_KEY=${envVars.GEMINI_API_KEY}`);
  }
