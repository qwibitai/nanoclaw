...
const OPENCLAW_AUTH_DIR =
  process.env.OPENCLAW_AUTH_DIR ||
  path.join(process.env.HOME || os.homedir(), '.openclaw', 'store', 'auth');

const logger = pino({
  level: 'warn', // Quiet logging - only show errors
});

function seedAuthFromOpenClaw(): void {
  const localCreds = path.join(AUTH_DIR, 'creds.json');
  const sourceCreds = path.join(OPENCLAW_AUTH_DIR, 'creds.json');
  if (fs.existsSync(localCreds) || !fs.existsSync(sourceCreds)) return;
  try {
    fs.mkdirSync(AUTH_DIR, { recursive: true });
    fs.cpSync(OPENCLAW_AUTH_DIR, AUTH_DIR, { recursive: true });
    console.log('Using existing WhatsApp credentials from OpenClaw setup.\n');
  } catch (err: any) {
    console.warn(`Could not copy OpenClaw credentials: ${err.message}`);
  }
}
