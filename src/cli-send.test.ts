...
describe('sender handling', () => {
  ...
  it('accepts sender with hyphens and underscores', () => {
    const result = sendMessage({ message: 'test', sender: 'my-tool_v2' });
    expect(result.sender).toBe('my-tool_v2');
  });

  it('rejects sender with spaces', () => {
    expect(() => sendMessage({ message: 'test', sender: 'my tool' })).toThrow(
      /Invalid sender name/,
    );
  });

  it('rejects sender with uppercase', () => {
    expect(() => sendMessage({ message: 'test', sender: 'MyTool' })).toThrow(
      /Invalid sender name/,
    );
  });

  it('rejects sender with special characters', () => {
    expect(() => sendMessage({ message: 'test', sender: 'tool@v1' })).toThrow(
      /Invalid sender name/,
    );
  });
});
