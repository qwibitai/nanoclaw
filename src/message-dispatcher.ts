...
async function processGroupMessages(chatJid: string): Promise<boolean> {
  ...
  const cmdResult = await handleSessionCommand({
    ...
    deps: {
      ...
      advanceCursor: (ts) => {
        state.lastAgentTimestamp[chatJid] = ts;
        saveState();
      },
      formatMessages,
      ...
      deleteSession,
      deleteInMemorySession: (groupFolder) => {
  ...
  if (output === 'error' || hadError) {
    ...
    saveState();
    log.warn('Agent error, rolled back message cursor for retry');
    return false;
  }
...
async function runAgent(
  ...
  const log = correlationId
    ? createCorrelationLogger(correlationId, { group: group.name })
    : logger;
...
/**
 * Poll for new messages and dispatch them to groups.
 */
...
/**
 * Wire the queue to use processGroupMessages.
 * Called during startup after all subsystems are initialized.
 */
