...
describe('extractCode', () => {
  it('accepts a bare 6-digit code', () => {
    expect(extractCode('103490', 'nanobot')).toBe('103490');
  });
  it('accepts 6-digit code after @botname', () => {
    expect(extractCode('@nanobot 100042', 'nanobot')).toBe('100042');
  });
  it('accepts legacy 4-digit codes while old pending records age out', () => {
    expect(extractCode('0349', 'nanobot')).toBe('0349');
  });
  it('rejects other numeric lengths', () => {
...
describe('createPairing', () => {
  it('generates a 6-digit code with an expiry', async () => {
...
describe('expiry', () => {
  it('marks expired pending records invalidated', async () => {
    const r = await createPairing('main');
    const stored = getPairing(r.code)!;
    stored.expiresAt = new Date(Date.now() - 1000).toISOString();
    fs.writeFileSync(path.join(tmpDir, 'pairings.json'), JSON.stringify({ pairings: [stored] }, null, 2));
