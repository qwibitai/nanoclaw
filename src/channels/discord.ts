...
import { downloadImageFromUrl, processImage } from '../image.js';
...
export class DiscordChannel implements Channel {
  ...
  async connect(): Promise<void> {
    ...
    this.client.on(Events.MessageCreate, async (message: Message) => {
      ...
      // Handle attachments — download images, use placeholders for other types
      if (message.attachments.size > 0) {
        ...
        for (const att of message.attachments.values()) {
          ...
          if (contentType.startsWith('image/')) {
            try {
              const buffer = await downloadImageFromUrl(att.url);
              ...
              parts.push(`[Image: ${att.name || 'image'}]`);
            }
      ...
      // Skip if there's still nothing to deliver
      if (!content) return;
