...
import { extractCode, tryConsume } from './telegram-pairing.js';
...
function extractReplyContext(raw: Record<string, unknown>): ReplyContext | null {
  if (!raw.reply_to_message || typeof raw.reply_to_message !== 'object') return null;
  const reply = raw.reply_to_message as {
    text?: string;
    caption?: string;
...
function createPairingInterceptor(
  ...
  return async (platformId, threadId, message) => {
    try {
      ...
      if (extractCode(text, botUsername) && !authorUserId) {
        log.warn('Telegram pairing code ignored — missing sender user id', { platformId });
        return;
      }
      ...
      const adminUserId = consumed.consumed?.adminUserId;
      if (!adminUserId) {
        log.warn('Telegram pairing consumed without sender user id — refusing owner grant', { platformId });
        return;
      }
...
function createTelegramChannel(channelType: string, token: string): ChannelAdapter {
  ...
  const bridge = createChatSdkBridge({
    ...
    maxTextLength: 4000,
    // Telegram caps captions on file sends at 1024 chars. Without this, the
    // adapter character-truncates the caption mid-entity and Telegram rejects
    // the message with a Markdown parse error. The bridge splits the caption
    // off cleanly and posts the remainder as follow-up text messages.
    maxCaptionLength: 1024,
    webhookRouteName: channelType,
  });
  ...
  return {
    ...
    async setup(hostConfig: ChannelSetup) {
      ...
      return withRetry(() => bridge.setup(intercepted), `${channelType}.bridge.setup`);
    },
  };
}

function registerTelegramChannel(channelType: string, envKey: string): void {
  registerChannelAdapter(channelType, {
    factory: () => {
      const env = readEnvFile([envKey]);
      const token = env[envKey];
      if (!token) return null;
      return createTelegramChannel(channelType, token);
    },
  });
}

registerTelegramChannel('telegram', 'TELEGRAM_BOT_TOKEN');
registerTelegramChannel('telegram-podcasts', 'PODCASTS_TELEGRAM_BOT_TOKEN');
registerTelegramChannel('telegram-analyst', 'THEDIUS_ANALYST_BOT_TOKEN');
registerTelegramChannel('telegram-builder', 'THEDIUS_BUILDER_BOT_TOKEN');
registerTelegramChannel('telegram-bytes', 'DA_BYTES_BOT_TOKEN');
