...
export function sanitizeTelegramLegacyMarkdown(input: string): string {
  ...
  // When delimiters are unbalanced, Telegram's parser would reject the message.
  // Escape stray chars rather than stripping them — stripping silently mangles
  // URLs that contain `_` (`telegram_main`/the-view → telegrammain/the-view) and
  // dropped chars in `*`-bearing tokens. `\_` and `\*` render as literal in
