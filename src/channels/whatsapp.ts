  ...
  factory: () => {
    ...
    async function connectSocket(): Promise<void> {
      ...
      sock = makeWASocket({
        ...
        // Defense-in-depth (originally jibot-code-5m2): Baileys' default
        // 60s init-query timeout trips reliably under event-loop pressure
        // (heavy IPC, large message bursts, slow downstream channels).
        // Doubling the budget absorbs transient stalls without changing
        // semantics. Cheap belt-and-braces; no harm if 60s would have
        // sufficed.
