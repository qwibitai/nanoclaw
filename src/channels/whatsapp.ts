...
export class WhatsAppChannel implements Channel {
    ...
    this.sock.ev.on('messages.upsert', async ({ messages }) => {
      for (const msg of messages) {
        ...
        if (groups[chatJid]) {
          ...
          // Download and save any media attachments
          const hasMedia =
          ...
          // On a dedicated number, fromMe reliably means the bot sent it (skip).
          // On a shared number, fromMe is true for ALL outbound including user's own messages,
          // so we can't skip based on fromMe alone.
          const isBotSentMedia = ASSISTANT_HAS_OWN_NUMBER && fromMe;
          let mediaFilePath: string | undefined;
          if (hasMedia && !isBotSentMedia) {
            mediaFilePath = await this.handleMediaMessage(
              msg,
              chatJid,
              group.folder,
              sender,
          ...
          const content = mediaFilePath
            ? captionOrText
              ? `${captionOrText}\n[File received: ${path.basename(mediaFilePath)}]`
              : `[File received: ${path.basename(mediaFilePath)}]`
            : captionOrText;
  ...
  async sendFile(
    ...
    if (!this.connected) {
      logger.warn(
        { jid, filePath },
        'WhatsApp not connected, cannot send file',
      );
  ...
  private async handleMediaMessage(
    ...
    const mediaTypeKey = msg.message.imageMessage
      ? 'images'
      : msg.message.documentMessage
        ? 'documents'
        : msg.message.videoMessage
    ...
    const originalName: string = msg.message.documentMessage?.fileName || '';
    ...
    const typeDir = path.join(GROUPS_DIR, groupFolder, 'files', mediaTypeKey);
    ...
    saveReceivedFile({
      id: msg.key.id || `${chatJid}_${Date.now()}`,
      chatJid,
      sender,
      senderName,
      filePath,
    ...
    logger.info(
      { chatJid, fileName, fileSize: buffer.length, mimeType },
      'Media file saved',
    );
