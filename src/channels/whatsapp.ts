...
export class WhatsAppChannel implements Channel {
    ...
    this.sock.ev.on('messages.upsert', async ({ messages }) => {
      for (const msg of messages) {
        ...
        if (groups[chatJid]) {
          ...
          // Skip voice messages sent by the bot itself (prevents echo loop in self-chat)
          if (isVoiceMessage(msg) && fromMe && !content) {
            continue;
          }
  ...
  async sendVoice(jid: string, audio: Buffer): Promise<void> {
    if (!this.connected) {
      logger.warn({ jid }, 'WA disconnected, cannot send voice');
      return;
    }
    const os = await import('os');
    const tmpFile = path.join(os.tmpdir(), `nanoclaw-tts-${Date.now()}.ogg`);
    try {
...
registerChannel('whatsapp', (opts: ChannelOpts) => {
  ...
  if (!fs.existsSync(path.join(authDir, 'creds.json'))) {
    logger.warn('WhatsApp: no auth credentials found');
    return null;
  }
