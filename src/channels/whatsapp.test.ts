...
describe('WhatsAppChannel', () => {
  ...
  describe('authentication', () => {
    it('emits ops event and resolves connect on QR (headless)', async () => {
      const opts = createTestOpts();
      const channel = new WhatsAppChannel(opts);
      ...
      // Emit QR code event (headless — no terminal to scan)
      fakeSocket._ev.emit('connection.update', { qr: 'some-qr-data' });

      // connect() should resolve even without opening (so cockpit can start)
      await p;

      expect(channel.isConnected()).toBe(false);
      expect(emitOpsEvent).toHaveBeenCalledWith('channel:status', expect.objectContaining({
        channel: 'whatsapp',
        status: 'auth_required',
      }));
    });

    it('does not reconnect after QR auth required', async () => {
      const opts = createTestOpts();
      const channel = new WhatsAppChannel(opts);
      ...
      // Connection close follows — should NOT trigger reconnect
      fakeSocket._ev.emit('connection.update', {
        connection: 'close',
        lastDisconnect: { error: { output: { statusCode: 428 } } },
      });

      // Give reconnect time to kick in (it shouldn't)
  ...
  describe('reconnection', () => {
    ...
    it('emits ops event on loggedOut instead of exiting', async () => {
      const opts = createTestOpts();
      const channel = new WhatsAppChannel(opts);
      ...
      await triggerMessages([
        {
          ...
          message: { conversation: 'Hello Flux' },
          pushName: 'Alice',
  ...
  describe('outgoing message queue', () => {
    it('sends message directly when connected', async () => {
      ...
      expect(fakeSocket.sendMessage).toHaveBeenCalledWith('test@g.us', { text: 'Flux: Hello' });
    });
    ...
    it('prefixes direct chat messages on shared number', async () => {
      ...
      expect(fakeSocket.sendMessage).toHaveBeenCalledWith('123@s.whatsapp.net', { text: 'Flux: Hello' });
    });
    ...
    it('flushes multiple queued messages in order', async () => {
      ...
      expect(fakeSocket.sendMessage).toHaveBeenNthCalledWith(1, 'test@g.us', { text: 'Flux: First' });
      expect(fakeSocket.sendMessage).toHaveBeenNthCalledWith(2, 'test@g.us', { text: 'Flux: Second' });
      expect(fakeSocket.sendMessage).toHaveBeenNthCalledWith(3, 'test@g.us', { text: 'Flux: Third' });
    });
  });
