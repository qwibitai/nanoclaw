...
function createTestOpts(
  ...
  return {
    ...
    onRegisterMain: vi.fn(async () => ({ status: 'registered' as const })),
...
function createAccessControlMock(overrides?: Record<string, any>) {
  return {
    isEnabled: vi.fn(() => true),
    isAdmin: vi.fn((id: string) => id === '99001'),
    isUserAllowed: vi.fn((id: string) => id === '99001'),
    getAdminUserId: vi.fn(() => '99001'),
    getAllowedUserIds: vi.fn(() => ['99002']),
    setAdmin: vi.fn(() => ({ ok: true, message: 'Admin set.' })),
    allowUser: vi.fn(() => ({ ok: true, message: 'Access granted.' })),
    removeUser: vi.fn(() => ({ ok: true, message: 'Access removed.' })),
    ...overrides,
  };
}
...
describe('TelegramChannel', () => {
  ...
  describe('connection lifecycle', () => {
    ...
    it('registers command and message handlers on connect', async () => {
      ...
      expect(currentBot().commandHandlers.has('myid')).toBe(true);
      expect(currentBot().commandHandlers.has('set_admin')).toBe(true);
      expect(currentBot().commandHandlers.has('allow_user')).toBe(true);
      expect(currentBot().commandHandlers.has('remove_user')).toBe(true);
      expect(currentBot().commandHandlers.has('list_users')).toBe(true);
      expect(currentBot().commandHandlers.has('register_main')).toBe(true);
  ...
  describe('text message handling', () => {
    ...
    it('blocks unauthorized users when access control is enabled', async () => {
      const accessControl = createAccessControlMock({
        isUserAllowed: vi.fn(() => false),
        getAdminUserId: vi.fn(() => '99001'),
      });
      const opts = createTestOpts({ accessControl });
      const channel = new TelegramChannel('test-token', opts);
      await channel.connect();

      const ctx = createTextCtx({ text: 'Can I use this?', fromId: 12345 });
      await triggerTextMessage(ctx);
      ...
      expect(accessControl.isUserAllowed).toHaveBeenCalledWith('12345');
    });
  ...
  describe('non-text messages', () => {
    ...
    it('blocks non-text messages from unauthorized users', async () => {
      const accessControl = createAccessControlMock({
        isUserAllowed: vi.fn(() => false),
      });
      const opts = createTestOpts({ accessControl });
      ...
      const ctx = createMediaCtx({ fromId: 12345 });
      await triggerMediaMessage('message:photo', ctx);
  ...
  describe('bot commands', () => {
    ...
    it('/myid replies with normalized Telegram user ID', async () => {
      const opts = createTestOpts();
      const channel = new TelegramChannel('test-token', opts);
      await channel.connect();

      const handler = currentBot().commandHandlers.get('myid')!;
      const ctx = { from: { id: 456789 }, reply: vi.fn() };
    ...
    it('/set_admin uses access control callback', async () => {
      const accessControl = createAccessControlMock({
        setAdmin: vi.fn(() => ({ ok: true, message: 'Admin set to 333.' })),
      });
      const opts = createTestOpts({ accessControl });
      const channel = new TelegramChannel('test-token', opts);
      await channel.connect();

      const handler = currentBot().commandHandlers.get('set_admin')!;
      const ctx = {
        from: { id: 111 },
        match: '333',
        message: { text: '/set_admin 333' },
        reply: vi.fn(),
      };

      await handler(ctx);

      expect(accessControl.setAdmin).toHaveBeenCalledWith('111', '333');
      expect(ctx.reply).toHaveBeenCalledWith('Admin set to 333.');
    });

    it('/allow_user grants access through access control callback', async () => {
      const accessControl = createAccessControlMock({
        allowUser: vi.fn(() => ({ ok: true, message: 'Access granted to user 333.' })),
      });
      const opts = createTestOpts({ accessControl });
      const channel = new TelegramChannel('test-token', opts);
      await channel.connect();

      const handler = currentBot().commandHandlers.get('allow_user')!;
      const ctx = {
        from: { id: 99001 },
        match: '333',
        message: { text: '/allow_user 333' },
        reply: vi.fn(),
      };

      await handler(ctx);

      expect(accessControl.allowUser).toHaveBeenCalledWith('99001', '333');
      expect(ctx.reply).toHaveBeenCalledWith('Access granted to user 333.');
    });

    it('/list_users shows admin and allowed users for admin requester', async () => {
      const accessControl = createAccessControlMock({
        isAdmin: vi.fn(() => true),
        getAdminUserId: vi.fn(() => '99001'),
        getAllowedUserIds: vi.fn(() => ['99002', '99003']),
      });
      const opts = createTestOpts({ accessControl });
      const channel = new TelegramChannel('test-token', opts);
      await channel.connect();

      const handler = currentBot().commandHandlers.get('list_users')!;
      const ctx = {
        from: { id: 99001 },
        reply: vi.fn(),
      };
      ...
      expect(ctx.reply).toHaveBeenCalledWith(
        expect.stringContaining('Admin: 99001'),
      );
      expect(ctx.reply).toHaveBeenCalledWith(
        expect.stringContaining('99002, 99003'),
      );
    });

    it('/list_users blocks non-admin requester', async () => {
      const accessControl = createAccessControlMock({
        isAdmin: vi.fn(() => false),
        getAdminUserId: vi.fn(() => '99001'),
      });
      ...
      expect(ctx.reply).toHaveBeenCalledWith(
        expect.stringContaining('Only admin (99001)'),
      );
    });

    it('/register_main registers current chat via callback', async () => {
      const onRegisterMain = vi.fn(async () => ({ status: 'registered' as const }));
      const opts = createTestOpts({ onRegisterMain });
      const channel = new TelegramChannel('test-token', opts);
      await channel.connect();

      const handler = currentBot().commandHandlers.get('register_main')!;
      const ctx = {
      ...
      expect(onRegisterMain).toHaveBeenCalledWith('tg:123456', 'Alice');
      expect(ctx.reply).toHaveBeenCalledWith(
        expect.stringContaining('Main chat registered'),
      );
    });

    it('/register_main blocks unauthorized user when access control is enabled', async () => {
      const accessControl = createAccessControlMock({
        isEnabled: vi.fn(() => true),
        isUserAllowed: vi.fn(() => false),
      });
      const onRegisterMain = vi.fn(async () => ({ status: 'registered' as const }));
      const opts = createTestOpts({ onRegisterMain, accessControl });
      const channel = new TelegramChannel('test-token', opts);
      await channel.connect();
      ...
      const ctx = {
        chat: { id: 123456, type: 'private' as const },
        from: { id: 222222, first_name: 'Unauthorized' },
        reply: vi.fn(),
      };

      await handler(ctx);

      expect(onRegisterMain).not.toHaveBeenCalled();
      expect(ctx.reply).toHaveBeenCalledWith(
        expect.stringContaining('not authorized'),
      );
    });

    it('/register_main reports conflict when another main chat exists', async () => {
      const onRegisterMain = vi.fn(async () => ({
        status: 'main-exists' as const,
        existingJid: 'tg:-100123',
        existingName: 'Ops Room',
      }));
      const opts = createTestOpts({ onRegisterMain });
      const channel = new TelegramChannel('test-token', opts);
      await channel.connect();
      ...
      expect(ctx.reply).toHaveBeenCalledWith(
        expect.stringContaining('tg:-100123'),
      );
      expect(ctx.reply).toHaveBeenCalledWith(
        expect.stringContaining('Ops Room'),
      );
    });
  });
