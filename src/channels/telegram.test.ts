...
// Mock group-folder so downloadToGroup doesn't touch the filesystem
vi.mock('../group-folder.js', () => ({
  resolveGroupFolderPath: vi.fn(() => '/tmp/test-group'),
  assertValidGroupFolder: vi.fn(),
}));

// Mock fs to prevent filesystem writes during tests
vi.mock('fs', () => ({
...
function createMediaCtx(overrides: {
  ...
  return {
    ...
    message: {
      ...
      // Default photo array for message:photo tests (smallest → largest)
      photo: [{ file_id: 'small_id', file_size: 1000 }, { file_id: 'large_id', file_size: 5000 }],
      ...(overrides.extra || {}),
    },
    me: { username: 'andy_ai_bot' },
  };
...
describe('TelegramChannel', () => {
  ...
  describe('non-text messages', () => {
    it('stores photo with placeholder', async () => {
      ...
      // getFile returns {file_path: undefined} by default → download returns null → fallback content
      expect(opts.onMessage).toHaveBeenCalledWith(
        'tg:100200300',
        expect.objectContaining({ content: '[Photo: too large to download]' }),
      );
    });

    it('downloads photo and stores path when getFile succeeds', async () => {
      const opts = createTestOpts();
      const channel = new TelegramChannel('test-token', opts);
      await channel.connect();

      currentBot().api.getFile.mockResolvedValueOnce({ file_path: 'photos/abc.jpg' });
      ...
      expect(opts.onMessage).toHaveBeenCalledWith(
        'tg:100200300',
        expect.objectContaining({ content: expect.stringContaining('uploads/photo_') }),
      );
    });
    ...
    it('stores photo with caption', async () => {
      ...
      expect(opts.onMessage).toHaveBeenCalledWith(
        'tg:100200300',
        expect.objectContaining({ content: expect.stringMatching(/\[Photo.*\] Look at this/) }),
      );
    });
    ...
    it('downloads small document and stores path', async () => {
      const opts = createTestOpts();
      const channel = new TelegramChannel('test-token', opts);
      await channel.connect();

      currentBot().api.getFile.mockResolvedValueOnce({ file_path: 'documents/report.pdf' });
      global.fetch = vi.fn().mockResolvedValueOnce({
        ok: true,
        arrayBuffer: async () => new ArrayBuffer(8),
      }) as any;
      ...
      const ctx = createMediaCtx({
        extra: { document: { file_id: 'doc123', file_name: 'report.pdf', file_size: 1024 } },
      });
      await triggerMediaMessage('message:document', ctx);

      expect(opts.onMessage).toHaveBeenCalledWith(
        'tg:100200300',
        expect.objectContaining({ content: expect.stringContaining('uploads/doc_') }),
      );
    });

    it('stores document as placeholder when too large to download', async () => {
      const opts = createTestOpts();
      const channel = new TelegramChannel('test-token', opts);
      await channel.connect();

      const ctx = createMediaCtx({
        extra: { document: { file_id: 'doc123', file_name: 'big.zip', file_size: 25 * 1024 * 1024 } },
      });
      await triggerMediaMessage('message:document', ctx);

      expect(opts.onMessage).toHaveBeenCalledWith(
        'tg:100200300',
        expect.objectContaining({ content: expect.stringContaining('too large to download') }),
      );
    });
    ...
    it('stores document with fallback name when filename missing', async () => {
      ...
      const ctx = createMediaCtx({ extra: { document: { file_id: 'doc123' } } });
      await triggerMediaMessage('message:document', ctx);

      expect(opts.onMessage).toHaveBeenCalledWith(
        'tg:100200300',
        expect.objectContaining({ content: expect.stringContaining('file') }),
      );
    });
  ...
  describe('sendMessage', () => {
    ...
    it('converts markdown tables to preformatted ASCII art', async () => {
      const opts = createTestOpts();
      const channel = new TelegramChannel('test-token', opts);
      await channel.connect();

      const table = '| 轮次 | 结果 |\n|------|------|\n| 第1轮 | [1, 2] |';
      await channel.sendMessage('tg:100200300', table);

      const call = currentBot().api.sendMessage.mock.calls[0];
      expect(call[2]).toEqual({ parse_mode: 'HTML' });
      // Should be wrapped in <pre> and contain box-drawing chars
      expect(call[1]).toContain('<pre>');
      expect(call[1]).toContain('┌');
      expect(call[1]).toContain('轮次');
      expect(call[1]).toContain('第1轮');
    });

    it('renders Markdown as Telegram HTML when sending messages', async () => {
      const opts = createTestOpts();
      const channel = new TelegramChannel('test-token', opts);
      await channel.connect();

      await channel.sendMessage('tg:100200300', '```python\nprint("hi")\n```');
    ...
    it('falls back to plain text when Telegram rejects HTML parsing', async () => {
      ...
      // Second call is plain text fallback (no parse_mode)
