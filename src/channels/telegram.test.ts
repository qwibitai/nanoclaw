...
describe('TelegramChannel', () => {
  ...
  describe('constructor', () => {
    ...
    it('uses the provided token for bot initialization', async () => {
      const channel = new TelegramChannel({ token: 'specific-token-123', channelOptions: createTestOpts() });
      await channel.connect();

      // The Grammy Bot mock receives the token as first constructor arg
      expect(currentBot().token).toBe('specific-token-123');
    });
  });
  ...
  describe('connection lifecycle', () => {
    it('resolves connect() when bot starts', async () => {
      ...
      const channel = new TelegramChannel({ token: 'test-token', channelOptions: opts });
