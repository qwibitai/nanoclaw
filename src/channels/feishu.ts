...
async function resolveSenderName(
  ...
  try {
    ...
    if (name) {
      senderNameCache.set(openId, {
        name,
        expireAt: Date.now() + SENDER_NAME_TTL_MS,
      });
      return name;
    }
  } catch {
...
function parsePostContent(rawContent: string): string {
  try {
    const parsed = JSON.parse(rawContent) as {
      title?: string;
      content?: Array<
        Array<{ tag: string; text?: string; href?: string; user_name?: string }>
      >;
    };
...
export class FeishuChannel implements Channel {
  ...
  private async handleMessage(data: {
    ...
    if (!this.lastMessageId.has(chatJid)) {
      this.lastMessageId.set(chatJid, {
  ...
  async sendMessage(jid: string, text: string): Promise<void> {
    ...
    try {
      ...
      if (res?.code !== 0) {
        logger.warn(
          { jid, code: res?.code, msg: res?.msg },
          'Feishu send returned error code',
        );
      } else {
