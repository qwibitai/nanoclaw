...
vi.mock('net', () => {
  const mockSocket = {
    ...
    write: vi.fn((data: string, _encoding: string, cb?: (err?: Error) => void) => {
      if (cb) cb();
      // Auto-respond to JSON-RPC requests so _rpc() resolves
      try {
        const req = JSON.parse(data);
        if (req.id != null) {
          setTimeout(() => {
            const response = JSON.stringify({ jsonrpc: '2.0', id: req.id, result: {} }) + '\n';
            mockSocket._emit('data', Buffer.from(response));
          }, 0);
        }
...
const { captured } = vi.hoisted(() => ({
  captured: { factory: null as ((opts: unknown) => unknown) | null },
}));
vi.mock('./registry.js', () => ({
...
describe('SignalChannel', () => {
  ...
  it('self-registers when env vars are set', () => {
    ...
    const instance = captured.factory!(opts);
