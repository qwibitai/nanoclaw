...
/**
 * Resolve a chat JID to its registered group.
 * For thread-specific JIDs like `tg:-100123:5`, falls back to the base JID `tg:-100123`.
 */
export function resolveGroup(chatJid: string): RegisteredGroup | undefined {
  const direct = registeredGroups[chatJid];
  if (direct) return direct;

  // Fall back: strip thread suffix for tg: JIDs
...
/**
 * Get session key for a chat JID.
 * Thread-specific JIDs get their own session to isolate conversation context.
 * e.g. folder "mygroup" + JID "tg:-100:5" → "mygroup:t5"
 */
function getSessionKey(chatJid: string, groupFolder: string): string {
  if (chatJid.startsWith('tg:')) {
    const parts = chatJid.split(':');
    if (parts.length === 3) {
...
async function runAgent(
  ...
  const sessionKey = getSessionKey(chatJid, group.folder);
