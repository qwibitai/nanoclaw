    ...
    import('./workflow-router.js')
      ...
      .catch((err) =>
        logger.warn(
          ...
          'Workflow router failed (non-blocking)',
        ),
      );
  }
...
async function main(): Promise<void> {
  ...
  if (!wizardDone()) {
    // Wizard mode: only start dashboard with wizard, skip everything else
    logger.info('Wizard mode: starting setup wizard only');
    initDatabase();
  ...
  // Start x402 payment handler (host-side, secure)
  startX402Handler();
