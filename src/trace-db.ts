...
export function purgeOldTraces(): void {
  ...
  db.prepare('DELETE FROM tool_calls WHERE trace_id IN (SELECT id FROM traces WHERE started_at < ?)').run(cutoff);
  db.prepare('DELETE FROM llm_calls WHERE trace_id IN (SELECT id FROM traces WHERE started_at < ?)').run(cutoff);
  db.prepare('DELETE FROM traces WHERE started_at < ?').run(cutoff);
}

export function upsertTrace(id: string, group: string, chatJid: string, isScheduled: boolean, promptPreview: string): void {
  db.prepare(`
    INSERT OR IGNORE INTO traces (id, group_folder, chat_jid, is_scheduled, prompt_preview, started_at)
    VALUES (?, ?, ?, ?, ?, ?)
  `).run(id, group, chatJid, isScheduled ? 1 : 0, promptPreview, new Date().toISOString());
}

export function finishTrace(id: string, status: string, error?: string): void {
  db.prepare('UPDATE traces SET finished_at = ?, status = ?, error = ? WHERE id = ?')
...
export function startLlmCall(traceId: string, inputTokens: number | null, model: string | null): void {
  const result = db.prepare(`
    INSERT INTO llm_calls (trace_id, started_at, input_tokens, model) VALUES (?, ?, ?, ?)
  `).run(traceId, new Date().toISOString(), inputTokens, model) as Database.RunResult;
  openLlmCalls.set(traceId, result.lastInsertRowid as number);
}

export function endLlmCall(traceId: string, outputTokens: number | null, stopReason: string | null): void {
  const rowId = openLlmCalls.get(traceId);
  if (rowId == null) return;
  db.prepare('UPDATE llm_calls SET finished_at = ?, output_tokens = ?, stop_reason = ? WHERE id = ?')
    .run(new Date().toISOString(), outputTokens, stopReason, rowId);
  openLlmCalls.delete(traceId);
}

export function startToolCall(traceId: string, toolId: string, toolName: string, isSubagent: boolean): void {
  const result = db.prepare(`
    INSERT INTO tool_calls (trace_id, tool_id, tool_name, is_subagent, started_at)
    VALUES (?, ?, ?, ?, ?)
  `).run(traceId, toolId, toolName, isSubagent ? 1 : 0, new Date().toISOString()) as Database.RunResult;
...
export function endToolCall(traceId: string, toolId: string, outputPreview: string): void {
  const rowId = openToolCalls.get(traceId)?.get(toolId);
  if (rowId == null) return;
  db.prepare('UPDATE tool_calls SET finished_at = ?, output_preview = ? WHERE id = ?')
...
export function getLlmCalls(traceId: string): unknown[] {
  return db.prepare('SELECT * FROM llm_calls WHERE trace_id = ? ORDER BY started_at').all(traceId);
}

export function getToolCalls(traceId: string): unknown[] {
  return db.prepare('SELECT * FROM tool_calls WHERE trace_id = ? ORDER BY started_at').all(traceId);
}
