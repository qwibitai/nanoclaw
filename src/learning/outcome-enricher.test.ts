...
describe('buildRulesBlock', () => {
  ...
  it('formats rules as a compact block', () => {
    mockQueryRules.mockReturnValue([{
      id: 'r1', rule: 'Refresh OAuth tokens before email ops',
      source: 'outcome_pattern', actionClasses: ['email.read'],
      groupId: 'g1', confidence: 0.8, evidenceCount: 3,
      createdAt: new Date().toISOString(), lastMatchedAt: new Date().toISOString(),
    }]);
    const result = buildRulesBlock('check my email', 'g1');
  ...
  it('calls markMatched for each injected rule', () => {
    mockQueryRules.mockReturnValue([{
      id: 'r1', rule: 'Use API not browser',
      source: 'outcome_pattern', actionClasses: ['github.read'],
      groupId: 'g1', confidence: 0.7, evidenceCount: 2,
      createdAt: new Date().toISOString(), lastMatchedAt: new Date().toISOString(),
    }]);
  ...
  it('truncates block to 500 characters', () => {
    ...
    mockQueryRules.mockReturnValue(
      Array.from({ length: 5 }, (_, i) => ({
        id: `r${i}`, rule: longRule, source: 'outcome_pattern',
        actionClasses: ['email.read'], groupId: 'g1', confidence: 0.8,
        evidenceCount: 1, createdAt: new Date().toISOString(),
