...
function msg(content: string, ts: string): NewMessage {
  return {
...
describe('hippocampus middleware', () => {
  it('extracts query terms from latest message + recent context only', () => {
  ...
  it('falls back across known recall endpoints on 404', async () => {
    const fetchMock: typeof fetch = vi.fn(async (input: string | URL) => {
      if (
      ...
      return new Response(
        JSON.stringify({
          memories: [{ text: 'Recovered from fallback endpoint' }],
        }),
        {
          status: 200,
          headers: { 'content-type': 'application/json' },
    ...
    const output = await injectRecallBlock(
      {
        ...
        messages: [msg('hello fallback test', '2026-03-06T10:06:00.000Z')],
        chatJid: 'chat-1',
        groupFolder: 'main',
      },
      fetchMock,
    ...
    expect(output).toContain('Recovered from fallback endpoint');
  });
