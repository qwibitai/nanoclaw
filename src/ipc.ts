...
import {
  handleToolEventIpc,
  ToolEventIpc,
} from './ipc/tool-events-handler.js';
...
export function startIpcWatcher(deps: IpcDeps): void {
  ...
  // Clean up stale tool events (>7 days) on startup
  const pruned = pruneToolEvents(7);
  if (pruned > 0) {
    logger.info({ pruned }, 'Pruned stale tool events (>7 days)');
  }
    ...
    try {
      ...
      try {
        await processJsonIpcDirectory({
          ...
          handle: async (data, log) =>
            handleToolEventIpc(data as ToolEventIpc, sourceGroup, log),
        });
      } catch (err) {
        logger.error(
          { err, sourceGroup },
  ...
  const processIpcFiles = async () => {
    ...
    // Process all group folders concurrently — each group is independently locked
    // so simultaneous task completions from parallel worker slots don't queue up.
    await Promise.all(
      groupFolders.map((sourceGroup) => {
        const isMain = folderIsMain.get(sourceGroup) === true;
        return processGroup(sourceGroup, isMain, registeredGroups);
      }),
    );
  };
  ...
  // Periodic cleanup: prune tool events older than 7 days (every 6 hours)
  setInterval(
    () => {
      const pruned = pruneToolEvents(7);
      if (pruned > 0) {
