...
import {
  ...
  IPC_POLL_INTERVAL,
  TIMEZONE,
  SWARM_POLICY as CONFIG_SWARM_POLICY,
} from './config.js';
          ...
          for (const file of messageFiles) {
            ...
            try {
              ...
              if (data.type === 'message' && data.chatJid && data.text) {
                if (!messagesByJid[data.chatJid])
                  messagesByJid[data.chatJid] = [];
          ...
          for (const chatJid of Object.keys(messagesByJid)) {
            ...
            if (
              ...
              for (const e of entries) {
                logger.warn(
                  { chatJid: e.data.chatJid, sourceGroup },
                  'Unauthorized IPC message attempt blocked',
                );
                try {
                  fs.unlinkSync(path.join(messagesDir, e.file));
                } catch {}
              }
              continue;
            }
            ...
            if (SWARM_POLICY === 'send_all' || entries.length === 1) {
              ...
              try {
                ...
                logger.info(
                  { chatJid, sourceGroup, count: entries.length },
                  'IPC synthesized message sent',
                );
              } catch (err) {
                logger.error(
                  { err },
                  'Synthesizer failed; falling back to send_all',
                );
                for (const e of entries) {
                  await deps.sendMessage(e.data.chatJid, e.data.text);
                }
