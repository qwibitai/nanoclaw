...
describe('migration028', () => {
  it('token_hmac UNIQUE rejects duplicate', () => {
    const db = makeDb();
    db.exec(`INSERT INTO dashboard_tokens (user_id, token_hmac, issued_at, expires_at) VALUES ('u1', 'hmac1', '2026-01-01', '2026-01-02')`);
    expect(() => db.exec(`INSERT INTO dashboard_tokens (user_id, token_hmac, issued_at, expires_at) VALUES ('u1', 'hmac1', '2026-01-01', '2026-01-02')`)).toThrow();
    db.close();
  });

  it('steer_idempotency UNIQUE(user_id, idempotency_key)', () => {
    const db = makeDb();
    db.exec(`INSERT INTO steer_idempotency (user_id, idempotency_key, task_id, message_id, text, reserved_at, request_hash) VALUES ('u1', 'k1', 't1', 'm1', 'hello', '2026-01-01', 'hash1')`);
    expect(() => db.exec(`INSERT INTO steer_idempotency (user_id, idempotency_key, task_id, message_id, text, reserved_at, request_hash) VALUES ('u1', 'k1', 't2', 'm2', 'hello2', '2026-01-01', 'hash2')`)).toThrow();
    db.close();
  });
