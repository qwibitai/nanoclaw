...
describe('migration 021: mnemon-recall-feedback', () => {
  it('test_migration_creates_table', () => {
    ...
    const row = db
      .prepare(`SELECT name FROM sqlite_master WHERE type='table' AND name='recall_outcomes'`)
      .get() as { name: string } | undefined;
    expect(row?.name).toBe('recall_outcomes');

    const versionRow = db
      .prepare(`SELECT name FROM schema_version WHERE name='mnemon-recall-feedback-v1'`)
      .get() as { name: string } | undefined;
  ...
  it('test_migration_is_idempotent', () => {
    ...
    const count = (
      db
        .prepare(`SELECT COUNT(*) AS c FROM schema_version WHERE name='mnemon-recall-feedback-v1'`)
        .get() as { c: number }
    ).c;
    expect(count).toBe(1);
  });
  ...
  it('test_all_16_columns_present', () => {
    const db = tracked(freshDb());
    runMnemonIngestMigrations(db);
    runMnemonRecallFeedbackMigration(db);
