...
export function startCustomize(description: string): void {
  if (isCustomizeActive()) {
    throw new Error("A customize session is already active. Commit or abort it first.");
  }
...
export function commitCustomize(): void {
  ...
  const pending = parse(fs.readFileSync(pendingPath, "utf-8")) as PendingCustomize;
