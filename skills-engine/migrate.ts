...
import { BASE_DIR, CUSTOM_DIR, CODECLAW_DIR } from './constants.js';
import { initCodeclawDir } from './init.js';
import { recordCustomModification } from './state.js';

export function initSkillsSystem(): void {
  initCodeclawDir();
  console.log('Skills system initialized. .codeclaw/ directory created.');
}
  ...
  try {
    ...
    if (diff.trim()) {
      ...
      const filesModified = [...diff.matchAll(/^diff -ruN .+ (.+)$/gm)]
        ...
        .filter((f) => !f.startsWith('.codeclaw'));
      ...
      console.log(
        'Custom modifications captured in .codeclaw/custom/migration.patch',
      );
    } else {
