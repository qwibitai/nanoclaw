...
describe("run-migrations", () => {
  ...
  function runMigrations(from: string, to: string): { stdout: string; exitCode: number } {
    try {
...
`,
    ...
    expect(result.results.map((r: any) => r.version)).toEqual(["1.1.0", "1.2.0", "1.3.0"]);

    // Verify execution order from the log file
    const log = fs.readFileSync(path.join(tmpDir, "migration-order.log"), "utf-8");
    expect(log.trim()).toBe("1.1.0\n1.2.0\n1.3.0");
  });

  it("reports failure and exits non-zero when a migration throws", () => {
    createMigration("1.1.0", `throw new Error('migration failed intentionally');`);
