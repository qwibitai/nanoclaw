...
import { cleanup, createMinimalState, createTempDir, setupNanoclawDir } from "./test-helpers.js";
...
describe("path-remap", () => {
  ...
  describe("resolvePathRemap", () => {
    ...
    it("ignores remap target that resolves through symlink outside project root", () => {
      const outsideDir = fs.mkdtempSync(path.join(path.dirname(tmpDir), "nanoclaw-remap-outside-"));
    ...
    it("throws when requested path itself escapes project root", () => {
      expect(() => resolvePathRemap("../../outside.txt", {})).toThrow(/escapes project root/i);
    });
  });
  ...
  describe("recordPathRemap", () => {
    ...
    it("rejects unsafe remap entries", () => {
      expect(() => recordPathRemap({ "src/a.ts": "../../outside.txt" })).toThrow(
