...
describe("replay", () => {
  ...
  describe("replaySkills", () => {
    it("replays a single skill from base", async () => {
      ...
      fs.writeFileSync(path.join(tmpDir, "src", "config.ts"), "modified content\n");
      ...
      expect(fs.readFileSync(path.join(tmpDir, "src", "telegram.ts"), "utf-8")).toBe(
        "telegram code\n",
      );

      // Modified file should be merged from base
      const config = fs.readFileSync(path.join(tmpDir, "src", "config.ts"), "utf-8");
    ...
    it("resets files to base before replay", async () => {
      ...
      // Current has drift
      fs.mkdirSync(path.join(tmpDir, "src"), { recursive: true });
      fs.writeFileSync(path.join(tmpDir, "src", "config.ts"), "drifted content\n");

      // Also a stale added file
      fs.writeFileSync(path.join(tmpDir, "src", "stale-add.ts"), "should be removed");
      ...
      expect(fs.readFileSync(path.join(tmpDir, "src", "stale-add.ts"), "utf-8")).toBe("fresh add");
    });
  });
});
