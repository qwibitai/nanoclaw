...
describe("file-ops", () => {
  ...
  it("rename success", () => {
    ...
    const result = executeFileOps([{ type: "rename", from: "old.ts", to: "new.ts" }], tmpDir);
  ...
  it("move success", () => {
    ...
    const result = executeFileOps([{ type: "move", from: "file.ts", to: "sub/file.ts" }], tmpDir);
    expect(result.success).toBe(true);
  ...
  it("delete success", () => {
    ...
    const result = executeFileOps([{ type: "delete", path: "remove-me.ts" }], tmpDir);
    expect(result.success).toBe(true);
  ...
  it("rename target exists produces error", () => {
    ...
    const result = executeFileOps([{ type: "rename", from: "a.ts", to: "b.ts" }], tmpDir);
    expect(result.success).toBe(false);
    expect(result.errors.length).toBeGreaterThan(0);
  });

  it("delete missing file produces warning not error", () => {
    const result = executeFileOps([{ type: "delete", path: "nonexistent.ts" }], tmpDir);
    expect(result.success).toBe(true);
  ...
  it("source missing produces error for rename", () => {
    const result = executeFileOps([{ type: "rename", from: "missing.ts", to: "new.ts" }], tmpDir);
    expect(result.success).toBe(false);
    expect(result.errors.length).toBeGreaterThan(0);
  });
  ...
  it("delete rejects symlink escape to outside project root", () => {
    ...
    const result = executeFileOps([{ type: "delete", path: "linkdir/victim.ts" }], tmpDir);
