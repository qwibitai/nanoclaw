...
describe("rebase", () => {
  ...
  it("rebase with one skill: patch created, state updated, rebased_at set", async () => {
    ...
    const stateContent = fs.readFileSync(path.join(tmpDir, ".nanoclaw", "state.yaml"), "utf-8");
    ...
    const workingContent = fs.readFileSync(path.join(tmpDir, "src", "index.ts"), "utf-8");
  ...
  it("rebase with multiple skills + custom mods: all collapsed into single patch", async () => {
    ...
    // File added by skill
    fs.writeFileSync(path.join(tmpDir, "src", "plugin.ts"), "export const plugin = true;\n");
    ...
    const patchContent = fs.readFileSync(path.join(tmpDir, ".nanoclaw", "combined.patch"), "utf-8");
  ...
  it("rebase with new base: conflict returns backupPending", async () => {
    ...
    fs.mkdirSync(path.join(tmpDir, "src"), { recursive: true });
    fs.writeFileSync(path.join(tmpDir, "src", "index.ts"), "const x = 42; // skill override\n");
    ...
    const newBase = path.join(tmpDir, "new-core");
    fs.mkdirSync(path.join(newBase, "src"), { recursive: true });
    fs.writeFileSync(path.join(newBase, "src", "index.ts"), "const x = 999; // core v2\n");
