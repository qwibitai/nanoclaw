...
export function createTempDir(): string {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'cambot-agent-test-'));
}

export function setupCambotAgentDir(tmpDir: string): void {
  fs.mkdirSync(path.join(tmpDir, '.cambot-agent', 'base', 'src'), { recursive: true });
  fs.mkdirSync(path.join(tmpDir, '.cambot-agent', 'backup'), { recursive: true });
}
