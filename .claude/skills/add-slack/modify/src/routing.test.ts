...
describe('JID ownership patterns', () => {
  ...
  it('Slack channel JID: starts with C/G/D and alphanumerics', () => {
    const publicChannel = 'C123ABC';
    const privateChannel = 'G123ABC';
    const dm = 'D123ABC';
    expect(/^[CDG][A-Z0-9]+$/.test(publicChannel)).toBe(true);
    expect(/^[CDG][A-Z0-9]+$/.test(privateChannel)).toBe(true);
    expect(/^[CDG][A-Z0-9]+$/.test(dm)).toBe(true);
  });
});
