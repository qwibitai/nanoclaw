...
describe('getAvailableGroups', () => {
  ...
  it('includes Slack channel JIDs', () => {
    storeChatMetadata('slack:C1234567890', '2024-01-01T00:00:01.000Z', '#general', 'slack', true);
    storeChatMetadata('user@s.whatsapp.net', '2024-01-01T00:00:02.000Z', 'User DM', 'whatsapp', false);

    const groups = getAvailableGroups();
    expect(groups).toHaveLength(1);
    expect(groups[0].jid).toBe('slack:C1234567890');
  });

  it('marks registered Slack channels correctly', () => {
    storeChatMetadata('slack:C1234567890', '2024-01-01T00:00:01.000Z', 'Slack Registered', 'slack', true);
    ...
    const groups = getAvailableGroups();
    const slackReg = groups.find((g) => g.jid === 'slack:C1234567890');
  ...
  it('mixes WhatsApp and Slack chats ordered by activity', () => {
    ...
    storeChatMetadata('slack:C555', '2024-01-01T00:00:03.000Z', '#slack-channel', 'slack', true);
    storeChatMetadata('wa2@g.us', '2024-01-01T00:00:02.000Z', 'WhatsApp 2', 'whatsapp', true);

    const groups = getAvailableGroups();
    expect(groups).toHaveLength(3);
    expect(groups[0].jid).toBe('slack:C555');
