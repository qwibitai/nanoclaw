...
describe("add-gmail skill package", () => {
  ...
  it("has channel file with self-registration", () => {
    const channelFile = path.join(skillDir, "add", "src", "channels", "gmail.ts");
  ...
  it("has agent-runner Gmail MCP server modification", () => {
    const arFile = path.join(skillDir, "modify", "container", "agent-runner", "src", "index.ts");
  ...
  it("has test file for the channel", () => {
    const testFile = path.join(skillDir, "add", "src", "channels", "gmail.test.ts");
