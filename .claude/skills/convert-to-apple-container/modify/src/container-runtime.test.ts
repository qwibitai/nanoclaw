...
describe('cleanupOrphans', () => {
  it('stops orphaned cambot-agent containers from JSON output', () => {
    // Apple Container ls returns JSON
    const lsOutput = JSON.stringify([
      { status: 'running', configuration: { id: 'cambot-agent-group1-111' } },
      { status: 'stopped', configuration: { id: 'cambot-agent-group2-222' } },
      { status: 'running', configuration: { id: 'cambot-agent-group3-333' } },
    ...
    // ls + 2 stop calls (only running cambot-agent- containers)
    ...
    expect(mockExecSync).toHaveBeenNthCalledWith(
      3,
      `${CONTAINER_RUNTIME_BIN} stop cambot-agent-group3-333`,
      { stdio: 'pipe' },
    );
    expect(logger.info).toHaveBeenCalledWith(
      { count: 2, names: ['cambot-agent-group1-111', 'cambot-agent-group3-333'] },
  ...
  it('continues stopping remaining containers when one stop fails', () => {
    const lsOutput = JSON.stringify([
      { status: 'running', configuration: { id: 'cambot-agent-a-1' } },
      { status: 'running', configuration: { id: 'cambot-agent-b-2' } },
    ]);
    mockExecSync.mockReturnValueOnce(lsOutput);
    // First stop fails
    mockExecSync.mockImplementationOnce(() => {
