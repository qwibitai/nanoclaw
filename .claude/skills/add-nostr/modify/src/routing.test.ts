...
describe('JID ownership patterns', () => {
  ...
  it('Nostr JID: starts with nostr:', () => {
    const jid = 'nostr:a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4e5f6a1b2';
    expect(jid.startsWith('nostr:')).toBe(true);
  });
});
...
describe('getAvailableGroups', () => {
  ...
  it('excludes Nostr DMs from group list (isGroup=false)', () => {
    storeChatMetadata(
      'nostr:abcdef1234567890abcdef1234567890abcdef1234567890abcdef1234567890',
    ...
    const groups = getAvailableGroups();
    // Nostr DMs are not groups, so only the WhatsApp group appears
    expect(groups).toHaveLength(1);
    expect(groups[0].jid).toBe('group@g.us');
  });
});
