---
name: add-ollama
description: Add Ollama MCP server so the container agent can list, pull, delete, inspect, and monitor local Ollama models.
---
...
**1. Add path variable** — immediately after the `mcpServerPath` line in `main()`, add:

```typescript
const ollamaMcpServerPath = path.join(__dirname, 'ollama-mcp-stdio.js');
```

**2. Add to `runQuery` function signature** — in the `runQuery` function parameters, after `mcpServerPath: string`, add:

```typescript
ollamaMcpServerPath: string,
```

**3. Add to `allowedTools`** — in the `allowedTools` array, after `'mcp__nanoclaw__*'`, add:

```typescript
'mcp__ollama__*',
```

**4. Add to `mcpServers`** — inside the `mcpServers` object, after the closing brace of the `nanoclaw` entry, add:
...
**5. Update the `runQuery` call site** — find the call to `runQuery(...)` in `main()` and add `ollamaMcpServerPath` after `mcpServerPath`:

```typescript
// Before:
const queryResult = await runQuery(prompt, sessionId, mcpServerPath, containerInput, sdkEnv, resumeAt);
// After:
const queryResult = await runQuery(prompt, sessionId, mcpServerPath, ollamaMcpServerPath, containerInput, sdkEnv, resumeAt);
```
...
The main NanoClaw process passes environment variables to agent containers explicitly. Open `src/container-runner.ts` and add the following block immediately before the `// Runtime-specific args` comment:
