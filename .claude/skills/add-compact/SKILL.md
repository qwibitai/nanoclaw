...
> **Note:** `upstream` is the remote pointing to `qwibitai/argus`. If using a different remote name, substitute accordingly.
...
1. Start Argus in dev mode: `npm run dev`
...
```bash
git clone <your-fork> /tmp/argus-test
cd /tmp/argus-test
...
- **No config file.** Argus's philosophy is customization through code changes, not configuration sprawl.
