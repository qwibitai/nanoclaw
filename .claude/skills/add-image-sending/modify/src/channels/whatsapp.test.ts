...
describe('WhatsAppChannel', () => {
  ...
  describe('sendImage', () => {
    it('sends image with prefixed caption on shared number', async () => {
      const opts = createTestOpts();
      const channel = new WhatsAppChannel(opts);
      await connectChannel(channel);

      const buf = Buffer.from('fake-image');
      await channel.sendImage('test@g.us', buf, 'look at this');

      expect(fakeSocket.sendMessage).toHaveBeenCalledWith('test@g.us', {
        image: buf,
        caption: 'Andy: look at this',
      });
    });
    ...
    it('sends image without caption', async () => {
      ...
      const buf = Buffer.from('fake-image');
      await channel.sendImage('test@g.us', buf);
    ...
    it('drops image silently when disconnected', async () => {
      const opts = createTestOpts();
      const channel = new WhatsAppChannel(opts);
      // Do not connect � channel starts disconnected

      const buf = Buffer.from('fake-image');
      await channel.sendImage('test@g.us', buf, 'caption');
