...
**DNS Configuration:** Apple Container runs in a Linux VM and requires DNS configuration for network access. NanoClaw automatically adds `--dns 8.8.8.8` to container runtime arguments, which routes DNS queries through the macOS host. This is required for web searches, API calls, and package installations within containers.
