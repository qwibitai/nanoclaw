---
name: x-integration
description: X (Twitter) integration for CodeClaw. Post tweets, like, reply, retweet, and quote. Use for setup, testing, or troubleshooting X functionality. Triggers on "setup x", "x integration", "twitter", "post tweet", "tweet".
---
...
> **Compatibility:** CodeClaw v1.0.0. Directory structure may change in future versions.
...
1. **CodeClaw is installed and running** - WhatsApp connected, service active
...
npm run build
launchctl kickstart -k gui/$(id -u)/com.codeclaw  # macOS
# Linux: systemctl --user restart codeclaw
# Verify: launchctl list | grep codeclaw (macOS) or systemctl --user status codeclaw (Linux)
```
...
| `CODECLAW_ROOT` | `process.cwd()` | Project root directory |
...
| `logs/codeclaw.log` | Service logs (contains X operation logs) | Ignored |
...
To integrate this skill into CodeClaw, make the following modifications:
...
All paths below are relative to project root (`CODECLAW_ROOT`).
...
**Verify success:**
```bash
launchctl list | grep codeclaw  # macOS — should show PID and exit code 0 or -
# Linux: systemctl --user status codeclaw
```
...
grep -i "x_post\|x_like\|x_reply\|handleXIpc" logs/codeclaw.log | tail -20

# Script errors
grep -i "error\|failed" logs/codeclaw.log | tail -20
```
...
docker run codeclaw-agent ls -la /app/src/skills/
```
