...
describe('JID ownership patterns', () => {
  ...
  it('Zulip JID: starts with zl:', () => {
    const jid = 'zl:general/bot-requests';
    expect(jid.startsWith('zl:')).toBe(true);
  });
});
...
describe('getAvailableGroups', () => {
  ...
  it('includes Zulip stream JIDs', () => {
    storeChatMetadata('zl:general/bot-requests', '2024-01-01T00:00:01.000Z', 'general > bot-requests', 'zulip', true);
    storeChatMetadata('user@s.whatsapp.net', '2024-01-01T00:00:02.000Z', 'User DM', 'whatsapp', false);

    const groups = getAvailableGroups();
    expect(groups).toHaveLength(1);
    expect(groups[0].jid).toBe('zl:general/bot-requests');
  });

  it('marks registered Zulip streams correctly', () => {
    storeChatMetadata('zl:general/bot-requests', '2024-01-01T00:00:01.000Z', 'Registered', 'zulip', true);
    storeChatMetadata('zl:general/other-topic', '2024-01-01T00:00:02.000Z', 'Unregistered', 'zulip', true);

    _setRegisteredGroups({
      'zl:general/bot-requests': {
        name: 'general > bot-requests',
        folder: 'zulip-main',
        trigger: '@Andy',
    ...
    const groups = getAvailableGroups();
    const reg = groups.find((g) => g.jid === 'zl:general/bot-requests');
    const unreg = groups.find((g) => g.jid === 'zl:general/other-topic');
  ...
  it('mixes WhatsApp and Zulip chats ordered by activity', () => {
    storeChatMetadata('wa@g.us', '2024-01-01T00:00:01.000Z', 'WhatsApp', 'whatsapp', true);
    storeChatMetadata('zl:general/bots', '2024-01-01T00:00:03.000Z', 'Zulip', 'zulip', true);
    storeChatMetadata('wa2@g.us', '2024-01-01T00:00:02.000Z', 'WhatsApp 2', 'whatsapp', true);

    const groups = getAvailableGroups();
    expect(groups).toHaveLength(3);
    expect(groups[0].jid).toBe('zl:general/bots');
