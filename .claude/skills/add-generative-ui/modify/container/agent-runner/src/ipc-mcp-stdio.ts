...
async function waitForCanvasResponse(
  requestId: string,
  timeoutMs = 15000,
  ...
  while (Date.now() - startedAt < timeoutMs) {
    if (fs.existsSync(responsePath)) {
      try {
        ...
        return {
          success: false,
          error: 'Invalid canvas response payload',
        };
      }
    }
...
- Main group can target another registered group with group_folder.`,
  ...
  async (args) => {
    ...
    if (!isMain && args.group_folder && args.group_folder !== groupFolder) {
      return {
        content: [{
          type: 'text' as const,
          text: `Non-main groups cannot target group_folder "${args.group_folder}".`,
        }],
        isError: true,
      };
    }
    ...
    const response = await waitForCanvasResponse(requestId);
    if (!response) {
      return {
        content: [{
          type: 'text' as const,
          text: `Timed out waiting for canvas update response (${requestId}).`,
        }],
        isError: true,
      };
    }
    ...
    if (response.success !== true) {
      ...
      const line = typeof response.line === 'number'
        ? ` (line ${response.line})`
        : '';
      return {
        content: [{
    ...
    const canvasUrl = typeof response.canvasUrl === 'string'
      ? response.canvasUrl
      : 'http://127.0.0.1:4318/canvas';
    const group = (
      response.group && typeof response.group === 'object'
        ? response.group as Record<string, unknown>
        : {}
    );
    const folder = typeof group.folder === 'string' ? group.folder : groupFolder;
    const revision = typeof response.revision === 'number' ? response.revision : 'n/a';

    return {
      content: [{
        type: 'text' as const,
        text: `Canvas updated for "${folder}" (revision: ${revision}). View: ${canvasUrl}`,
      }],
    };
  },
);
