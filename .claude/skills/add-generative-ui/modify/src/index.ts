...
function findGroupByFolder(groupFolder: string): {
  jid: string;
...
function resolveTargetGroupFolder(
  requested: string | undefined,
  sourceGroup: string,
  isMain: boolean,
): { ok: true; groupFolder: string } | { ok: false; error: string } {
  const target = requested || sourceGroup;
  const targetGroup = findGroupByFolder(target);

  if (!targetGroup) {
    return {
      ok: false,
      error: `Unknown group folder: ${target}`,
    };
  }
  ...
  if (!isMain && target !== sourceGroup) {
    return {
      ok: false,
      error: `Unauthorized canvas update for target group: ${target}`,
    };
  }
...
async function updateCanvasState(
  ...
  const target = resolveTargetGroupFolder(data.groupFolder, sourceGroup, isMain);
  if (!target.ok) {
    return {
      success: false,
      error: target.error,
  ...
  const targetGroup = findGroupByFolder(target.groupFolder);
  if (!targetGroup) {
    return {
      success: false,
      error: `Unknown group folder: ${target.groupFolder}`,
    };
  }
...
async function main(): Promise<void> {
  ...
  try {
    ...
    logger.error({ err, port: GENUI_PORT }, 'Failed to start canvas server');
    canvasServer = null;
  }
  ...
  startIpcWatcher({
    ...
    updateCanvas: updateCanvasState,
  });
