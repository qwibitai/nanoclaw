...
let deps: IpcDeps;
let updateCanvasMock: ReturnType<typeof vi.fn>;
...
beforeEach(() => {
  ...
  deps = {
    ...
    getAvailableGroups: () => [],
    writeGroupsSnapshot: () => {},
    updateCanvas: async () => ({ success: true }),
  };

  updateCanvasMock = vi.fn(async () => ({ success: true }));
  deps.updateCanvas = updateCanvasMock;
});
...
// --- update_canvas authorization ---

describe('update_canvas authorization', () => {
  it('forwards source group and main role to updateCanvas deps handler', async () => {
    await processTaskIpc(
      {
        type: 'update_canvas',
        requestId: 'req-1',
  ...
  it('passes non-main context for enforcement inside updateCanvas handler', async () => {
    await processTaskIpc(
      {
        type: 'update_canvas',
        requestId: 'req-2',
