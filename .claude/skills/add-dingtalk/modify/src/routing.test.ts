...
describe('JID ownership patterns', () => {
  ...
  it('DingTalk JID: starts with dd:', () => {
    const jid = 'dd:cidABCDEF123==';
    expect(jid.startsWith('dd:')).toBe(true);
  });
...
describe('getAvailableGroups', () => {
  ...
  it('includes DingTalk group chat JIDs', () => {
    storeChatMetadata('dd:cidABCDEF123==', '2024-01-01T00:00:01.000Z', 'DingTalk Group', 'dingtalk', true);
    storeChatMetadata('user@s.whatsapp.net', '2024-01-01T00:00:02.000Z', 'WA DM', 'whatsapp', false);

    const groups = getAvailableGroups();
    expect(groups).toHaveLength(1);
    expect(groups[0].jid).toBe('dd:cidABCDEF123==');
  });

  it('marks registered DingTalk chats correctly', () => {
    storeChatMetadata('dd:cid001', '2024-01-01T00:00:01.000Z', 'DD Registered', 'dingtalk', true);
    storeChatMetadata('dd:cid002', '2024-01-01T00:00:02.000Z', 'DD Unregistered', 'dingtalk', true);

    _setRegisteredGroups({
      'dd:cid001': {
        name: 'DD Registered',
        folder: 'dingtalk-cid001',
        trigger: '@Andy',
    ...
    const groups = getAvailableGroups();
    const ddReg = groups.find((g) => g.jid === 'dd:cid001');
    const ddUnreg = groups.find((g) => g.jid === 'dd:cid002');

    expect(ddReg?.isRegistered).toBe(true);
    expect(ddUnreg?.isRegistered).toBe(false);
  });
  ...
  it('mixes WhatsApp and DingTalk chats ordered by activity', () => {
    storeChatMetadata('wa@g.us', '2024-01-01T00:00:01.000Z', 'WhatsApp', 'whatsapp', true);
    storeChatMetadata('dd:cid001', '2024-01-01T00:00:03.000Z', 'DingTalk', 'dingtalk', true);
    storeChatMetadata('wa2@g.us', '2024-01-01T00:00:02.000Z', 'WhatsApp 2', 'whatsapp', true);

    const groups = getAvailableGroups();
    expect(groups).toHaveLength(3);
    expect(groups[0].jid).toBe('dd:cid001');
