...
describe('dingtalk skill package', () => {
  ...
  it('has all files declared in adds', () => {
    const addFile = path.join(skillDir, 'add', 'src', 'channels', 'dingtalk.ts');
    ...
    const testFile = path.join(skillDir, 'add', 'src', 'channels', 'dingtalk.test.ts');
  ...
  it('has all files declared in modifies', () => {
    ...
    expect(indexContent).toContain('DingTalkChannel');
    expect(indexContent).toContain('DINGTALK_CLIENT_ID');
    expect(indexContent).toContain('DINGTALK_ONLY');
    expect(indexContent).toContain('findChannel');
    expect(indexContent).toContain('channels: Channel[]');

    const configContent = fs.readFileSync(configFile, 'utf-8');
    expect(configContent).toContain('DINGTALK_CLIENT_ID');
    expect(configContent).toContain('DINGTALK_CLIENT_SECRET');
    expect(configContent).toContain('DINGTALK_ONLY');
  });
  ...
  it('modified index.ts includes DingTalk channel creation', () => {
    const content = fs.readFileSync(
      path.join(skillDir, 'modify', 'src', 'index.ts'),
      'utf-8',
    );
    ...
    expect(content).toContain('channels.push(dingtalk)');

    // Conditional channel creation
    expect(content).toContain('if (!DINGTALK_ONLY)');
    expect(content).toContain('if (DINGTALK_CLIENT_ID && DINGTALK_CLIENT_SECRET)');
  ...
  it('modified config.ts preserves all existing exports', () => {
    ...
    // DingTalk exports added
    expect(content).toContain('export const DINGTALK_CLIENT_ID');
    expect(content).toContain('export const DINGTALK_CLIENT_SECRET');
    expect(content).toContain('export const DINGTALK_ROBOT_CODE');
    expect(content).toContain('export const DINGTALK_ALLOWED_USERS');
    expect(content).toContain('export const DINGTALK_ALLOWED_GROUPS');
    expect(content).toContain('export const DINGTALK_ONLY');
  });

  it('modified routing.test.ts includes DingTalk JID tests', () => {
    const content = fs.readFileSync(
      path.join(skillDir, 'modify', 'src', 'routing.test.ts'),
      'utf-8',
    );

    expect(content).toContain("DingTalk JID: starts with dd:");
    expect(content).toContain("dd:cidABCDEF123==");
