...
// Mock config — GROUPS_DIR added for image processing
vi.mock('../config.js', () => ({
  ASSISTANT_NAME: 'Andy',
...
// Mock image utilities from add-image-vision
vi.mock('../image.js', () => ({
...
// Mock global fetch for Discord CDN downloads
...
describe('DiscordChannel', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    // Restore default fetch mock: returns a valid image buffer
    mockFetch.mockResolvedValue({
      arrayBuffer: vi.fn().mockResolvedValue(new ArrayBuffer(8)),
    });
    // Restore default processImage mock
  ...
  describe('image attachments', () => {
    it('downloads and processes image attachments', async () => {
      ...
      expect(mockFetch).toHaveBeenCalledWith('https://cdn.discordapp.com/attachments/photo.png');
      expect(processImage).toHaveBeenCalledWith(
        expect.any(Buffer),
        path.join('/tmp/test-groups', 'test-server'),
        '',
      );
      expect(opts.onMessage).toHaveBeenCalledWith(
        'dc:1234567890123456',
    ...
    it('includes text content with processed image', async () => {
      const opts = createTestOpts();
      const channel = new DiscordChannel('test-token', opts);
      await channel.connect();
      ...
      expect(opts.onMessage).toHaveBeenCalledWith(
        'dc:1234567890123456',
        expect.objectContaining({
          content: 'Check this out\n[Image: attachments/img-test.jpg]',
        }),
      );
    });

    it('falls back to placeholder on image download failure', async () => {
    ...
    it('falls back to placeholder when processImage returns null', async () => {
      ...
      const attachments = new Map([
        ['att1', {
          name: 'corrupt.jpg',
          contentType: 'image/jpeg',
          url: 'https://cdn.discordapp.com/attachments/corrupt.jpg',
        }],
      ]);
      const msg = createMessage({ content: '', attachments, guildName: 'Server' });
      await triggerMessage(msg);
      ...
      expect(opts.onMessage).toHaveBeenCalledWith(
        'dc:1234567890123456',
        expect.objectContaining({ content: '[Image: corrupt.jpg]' }),
      );
    });

    it('does not download images for unregistered channels', async () => {
      const opts = createTestOpts();
      const channel = new DiscordChannel('test-token', opts);
      await channel.connect();
  ...
  describe('non-image attachments', () => {
    ...
    it('handles multiple attachments (image processed, file as placeholder)', async () => {
      const opts = createTestOpts();
      const channel = new DiscordChannel('test-token', opts);
      await channel.connect();
    ...
    it('does not call fetch for non-image attachments', async () => {
      const opts = createTestOpts();
      const channel = new DiscordChannel('test-token', opts);
      await channel.connect();

      const attachments = new Map([
        ['att1', { name: 'doc.pdf', contentType: 'application/pdf', url: 'https://cdn.discordapp.com/doc.pdf' }],
      ]);
      const msg = createMessage({ content: '', attachments, guildName: 'Server' });
      await triggerMessage(msg);
