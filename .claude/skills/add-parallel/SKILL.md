...
Adds Parallel AI MCP integration to Argus for advanced web research capabilities.
...
- **Non-blocking Design** - Uses Argus scheduler for result polling (no container blocking)

## Prerequisites

User must have:
1. Parallel AI API key from https://platform.parallel.ai
2. Argus already set up and running
...
Add Parallel AI MCP servers after the argus server:
```typescript
const mcpServers: Record<string, any> = {
  argus: ipcMcp
};
...
3. Create a polling scheduled task using `mcp__argus__schedule_task`:
   ```
   ...
   3. Send results to user with mcp__argus__send_message
   4. Use mcp__argus__complete_scheduled_task to mark this task as done
...
Verify the build:
```bash
echo '{}' | docker run -i --entrypoint /bin/echo argus-agent:latest "Container OK"
```
...
```bash
sleep 3
launchctl list | grep argus  # macOS
...
- Check task runs: `tail -f logs/argus.log | grep "scheduled task"`
...
5. Restart: `launchctl kickstart -k gui/$(id -u)/com.argus` (macOS) or `systemctl --user restart argus` (Linux)
