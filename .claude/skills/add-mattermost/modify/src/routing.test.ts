...
describe('JID ownership patterns', () => {
  ...
  it('Mattermost JID: starts with mm:', () => {
    const jid = 'mm:abcdef1234567890xyz1234567890xyz';
...
describe('getAvailableGroups', () => {
  ...
  it('includes Mattermost channel JIDs', () => {
    storeChatMetadata('mm:abcdef1234567890xyz1234567890xyz', '2024-01-01T00:00:01.000Z', 'Mattermost Channel', 'mattermost', true);
    storeChatMetadata('user@s.whatsapp.net', '2024-01-01T00:00:02.000Z', 'User DM', 'whatsapp', false);

    const groups = getAvailableGroups();
    expect(groups).toHaveLength(1);
    expect(groups[0].jid).toBe('mm:abcdef1234567890xyz1234567890xyz');
  });

  it('marks registered Mattermost channels correctly', () => {
    storeChatMetadata('mm:registered123', '2024-01-01T00:00:01.000Z', 'MM Registered', 'mattermost', true);
    storeChatMetadata('mm:unregistered456', '2024-01-01T00:00:02.000Z', 'MM Unregistered', 'mattermost', true);

    _setRegisteredGroups({
      'mm:registered123': {
        name: 'MM Registered',
        folder: 'mm-registered',
        trigger: '@Andy',
    ...
    const groups = getAvailableGroups();
    const mmReg = groups.find((g) => g.jid === 'mm:registered123');
    const mmUnreg = groups.find((g) => g.jid === 'mm:unregistered456');

    expect(mmReg?.isRegistered).toBe(true);
    expect(mmUnreg?.isRegistered).toBe(false);
  });
  ...
  it('mixes WhatsApp and Mattermost chats ordered by activity', () => {
    storeChatMetadata('wa@g.us', '2024-01-01T00:00:01.000Z', 'WhatsApp', 'whatsapp', true);
    storeChatMetadata('mm:mmid123', '2024-01-01T00:00:03.000Z', 'Mattermost', 'mattermost', true);
    storeChatMetadata('wa2@g.us', '2024-01-01T00:00:02.000Z', 'WhatsApp 2', 'whatsapp', true);

    const groups = getAvailableGroups();
    expect(groups).toHaveLength(3);
    expect(groups[0].jid).toBe('mm:mmid123');
