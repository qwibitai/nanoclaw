---
name: customize
description: Add new capabilities or modify NanoClaw behavior. Use when user wants to add channels (Discord, email input), change triggers, add integrations, modify the router, or make any other customizations. This is an interactive skill that asks questions to understand what the user wants.
---
...
Questions to ask:
- Which channel? (Discord, email, SMS, etc.)
...
1. Ask: "Should Discord use the same @Andy trigger, or a different one?"
2. Ask: "Should Discord messages create separate conversation contexts, or share with Telegram groups?"
3. Create `src/channels/discord.ts` implementing the `Channel` interface (see `src/channels/telegram.ts`)
