---
name: add-api-server
description: Expose your NanoClaw agent as an OpenAI-compatible HTTP API. Other tools (OpenClaw, Cursor, Continue, curl) can talk to your agent via standard /v1/chat/completions endpoint.
version: 1.0.0
author: Robby (HiFi Team Graz)
---
...
- User wants other tools (OpenClaw, Cursor, Continue, Open WebUI) to talk to this agent
...
Ask the user:
- **Port** (default: 8643)
- **API key** (default: none — open access within Tailscale/local network)
- **System prompt** (default: agent's personality from CLAUDE.md)
...
Write `/workspace/group/api-server/system-prompt.txt` with the system prompt (from CLAUDE.md or user-provided).
...
### From curl
```bash
curl -X POST http://TAILSCALE_IP:8643/v1/chat/completions \
  -H 'Content-Type: application/json' \
  -d '{"model":"nanoclaw-agent","messages":[{"role":"user","content":"What time is it?"}]}'
```

### From OpenClaw (as custom provider)
...
```json
{
  "providers": {
    "nanoclaw": {
      "type": "openai",
      "baseUrl": "http://TAILSCALE_IP:8643/v1",
      "apiKey": "optional-key"
    }
  }
}
```
Then create an agent with `"provider": "nanoclaw"` and bind it to a Telegram topic.

### From Python
```python
from openai import OpenAI
client = OpenAI(base_url="http://TAILSCALE_IP:8643/v1", api_key="optional")
...
- **Auth:** `claude -p` uses OAuth. If auth expires, the server returns 500. Re-login with `/login`.
- **Concurrency:** Each request spawns a `claude -p` process. Not a production API — single-agent endpoint for personal/local use.
