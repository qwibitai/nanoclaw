...
export class TelegramChannel implements Channel {
  ...
  async connect(): Promise<void> {
    ...
    this.bot.on('message_reaction', async (ctx) => {
      ...
      const reactorId = ctx.messageReaction.user?.id?.toString()
        || ctx.messageReaction.actor_chat?.id?.toString()
        || 'unknown';
      const reactorName = ctx.messageReaction.user?.username
      ...
      for (const reaction of ctx.messageReaction.old_reaction) {
        if (
          reaction.type === 'emoji' &&
          !ctx.messageReaction.new_reaction.find(r => r.type === 'emoji' && r.emoji === reaction.emoji)
        ) {
          storeReaction({
            message_id: messageId,
            message_chat_jid: chatId,
  ...
  async sendReaction(
    ...
    try {
      await this.bot.api.raw.sendReaction({
        chat_id: numericId,
        message_id: messageId,
