...
This skill adds Discord support to CamBot-Agent using the skills engine for deterministic code changes, then walks through interactive setup.

## Phase 1: Pre-flight

### Check if already applied

Read `.cambot-agent/state.yaml`. If `discord` is in `applied_skills`, skip to Phase 3 (Setup). The code changes are already in place.
...
4. Service is running: `launchctl list | grep cambot-agent`
...
- @mention translation (Discord `<@botId>` → CamBot-Agent trigger format)
