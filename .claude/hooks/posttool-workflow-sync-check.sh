...
if printf '%s\n' "$changed" | rg -q '^(CLAUDE\.md|AGENTS\.md|DOCS\.md|docs/|\.codex/config\.toml|\.codex/agents/|\.claude/settings\.local\.json|\.claude/hooks/|scripts/check-tooling-governance\.sh|scripts/check-docs-hygiene\.sh)'; then
  workflow_out=""
  mirror_out=""
  tooling_out=""
  docs_out=""
  ...
  docs_out="$(bash scripts/check-docs-hygiene.sh 2>&1)" || docs_rc=$?

  if [ "$workflow_rc" -ne 0 ] || [ "$mirror_rc" -ne 0 ] || [ "$tooling_rc" -ne 0 ] || [ "$docs_rc" -ne 0 ]; then
    echo "posttool warning: workflow/mirror/tooling/docs checks are failing after edits" >&2
    [ -n "$workflow_out" ] && echo "$workflow_out" >&2
    [ -n "$mirror_out" ] && echo "$mirror_out" >&2
    [ -n "$tooling_out" ] && echo "$tooling_out" >&2
    [ -n "$docs_out" ] && echo "$docs_out" >&2
