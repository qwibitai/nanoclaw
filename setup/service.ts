...
WantedBy=${runningAsRoot ? 'multi-user.target' : 'default.target'}`;
  ...
  // Enable linger so the user service survives after logout (non-root only)
  if (!runningAsRoot) {
    try {
      execSync(`loginctl enable-linger ${os.userInfo().username}`, { stdio: 'ignore' });
      logger.info('loginctl enable-linger succeeded');
    } catch (err) {
      logger.warn({ err }, 'loginctl enable-linger failed (may not be available)');
    }
  }
