...
<dict>
    ...
    <string>${projectRoot}/logs/argus.log</string>
    <key>StandardErrorPath</key>
    <string>${projectRoot}/logs/argus.error.log</string>
</dict>
</plist>`;
  ...
  try {
    ...
    serviceLoaded = output.includes('com.argus');
  } catch {
...
/**
 * Kill any orphaned argus node processes left from previous runs or debugging.
...
function killOrphanedProcesses(projectRoot: string): void {
  try {
    ...
    logger.info('Stopped any orphaned argus processes');
  } catch {
...
function setupSystemd(
  ...
  if (runningAsRoot) {
    unitPath = '/etc/systemd/system/argus.service';
    ...
    unitPath = path.join(unitDir, 'argus.service');
    systemctlPrefix = 'systemctl --user';
  }

  const unit = `[Unit]
Description=Argus Personal Assistant
...
StandardOutput=append:${projectRoot}/logs/argus.log
StandardError=append:${projectRoot}/logs/argus.error.log
...
WantedBy=${runningAsRoot ? 'multi-user.target' : 'default.target'}`;
  ...
  // Kill orphaned argus processes to avoid channel connection conflicts
  ...
  try {
    execSync(`${systemctlPrefix} enable argus`, { stdio: 'ignore' });
  } catch (err) {
    logger.error({ err }, 'systemctl enable failed');
  }

  try {
    execSync(`${systemctlPrefix} start argus`, { stdio: 'ignore' });
  } catch (err) {
  ...
  let serviceLoaded = false;
  try {
    execSync(`${systemctlPrefix} is-active argus`, { stdio: 'ignore' });
    serviceLoaded = true;
  } catch {
    // Not active
  }
...
function setupNohupFallback(
  ...
  const wrapperPath = path.join(projectRoot, 'start-argus.sh');
  const pidFile = path.join(projectRoot, 'argus.pid');

  const lines = [
    '#!/bin/bash',
    '# start-argus.sh — Start Argus without systemd',
    ...
    '    echo "Stopping existing Argus (PID $OLD_PID)..."',
    ...
    `  >> ${JSON.stringify(projectRoot + '/logs/argus.log')} \\`,
    `  2>> ${JSON.stringify(projectRoot + '/logs/argus.error.log')} &`,
    '',
    `echo $! > ${JSON.stringify(pidFile)}`,
    'echo "Argus started (PID $!)"',
    `echo "Logs: tail -f ${projectRoot}/logs/argus.log"`,
  ];
