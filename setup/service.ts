...
function setupLaunchd(projectRoot: string, nodePath: string, homeDir: string): void {
  const plistPath = path.join(homeDir, 'Library', 'LaunchAgents', 'com.codeclaw.plist');
...
<dict>
  ...
  try {
    ...
    serviceLoaded = output.includes('com.codeclaw');
  } catch {
...
function killOrphanedProcesses(projectRoot: string): void {
  try {
    ...
    logger.info('Stopped any orphaned codeclaw processes');
  } catch {
...
function setupSystemd(projectRoot: string, nodePath: string, homeDir: string): void {
  ...
  if (runningAsRoot) {
    unitPath = '/etc/systemd/system/codeclaw.service';
    ...
    unitPath = path.join(unitDir, 'codeclaw.service');
...
WantedBy=${runningAsRoot ? 'multi-user.target' : 'default.target'}`;
  ...
  try {
    execSync(`${systemctlPrefix} enable codeclaw`, { stdio: 'ignore' });
  } catch (err) {
    logger.error({ err }, 'systemctl enable failed');
  }

  try {
    execSync(`${systemctlPrefix} start codeclaw`, { stdio: 'ignore' });
  } catch (err) {
  ...
  let serviceLoaded = false;
  try {
    execSync(`${systemctlPrefix} is-active codeclaw`, { stdio: 'ignore' });
    serviceLoaded = true;
  } catch {
    // Not active
  }
...
function setupNohupFallback(projectRoot: string, nodePath: string, homeDir: string): void {
  ...
  const wrapperPath = path.join(projectRoot, 'start-codeclaw.sh');
  const pidFile = path.join(projectRoot, 'codeclaw.pid');

  const lines = [
    '#!/bin/bash',
    '# start-codeclaw.sh — Start CodeClaw without systemd',
    ...
    '    echo "Stopping existing CodeClaw (PID $OLD_PID)..."',
    ...
    `  >> ${JSON.stringify(projectRoot + '/logs/codeclaw.log')} \\`,
    `  2>> ${JSON.stringify(projectRoot + '/logs/codeclaw.error.log')} &`,
    '',
    `echo $! > ${JSON.stringify(pidFile)}`,
    'echo "CodeClaw started (PID $!)"',
    `echo "Logs: tail -f ${projectRoot}/logs/codeclaw.log"`,
  ];
