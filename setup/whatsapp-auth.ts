...
import { OPENCLAW_AUTH_DIR } from '../src/config.js';
...
function seedAuthFromOpenClaw(projectRoot: string): boolean {
  const localAuthDir = path.join(projectRoot, 'store', 'auth');
  const localCreds = path.join(localAuthDir, 'creds.json');
  if (fs.existsSync(localCreds)) return false;

  const sourceCreds = path.join(OPENCLAW_AUTH_DIR, 'creds.json');
  if (!fs.existsSync(sourceCreds)) return false;

  try {
    fs.mkdirSync(localAuthDir, { recursive: true });
    fs.cpSync(OPENCLAW_AUTH_DIR, localAuthDir, { recursive: true });
    return true;
  } catch (err) {
    logger.warn(
      { err, source: OPENCLAW_AUTH_DIR, destination: localAuthDir },
...
export async function run(args: string[]): Promise<void> {
  ...
  if (seedAuthFromOpenClaw(projectRoot)) {
    const seededPhone = getPhoneNumber(projectRoot);
    emitAuthStatus('openclaw-sync', 'seeded_from_openclaw', 'success', {
      SOURCE: OPENCLAW_AUTH_DIR,
      ...(seededPhone ? { PHONE_NUMBER: seededPhone } : {}),
    });
    return;
  }
