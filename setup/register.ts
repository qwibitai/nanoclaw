  ...
  for (let i = 0; i < args.length; i++) {
    switch (args[i]) {
      ...
      case '--is-main':
        ...
        break;
      case '--assistant-name':
        result.assistantName = args[++i] || 'Jarvis';
        break;
    }
  }
...
export async function run(args: string[]): Promise<void> {
  ...
  if (parsed.assistantName !== 'Jarvis') {
    logger.info(
      { from: 'Jarvis', to: parsed.assistantName },
    ...
    for (const mdFile of mdFiles) {
      if (fs.existsSync(mdFile)) {
        ...
        content = content.replace(/^# Jarvis$/m, `# ${parsed.assistantName}`);
