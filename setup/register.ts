  ...
  for (let i = 0; i < args.length; i++) {
    switch (args[i]) {
      ...
      case '--is-main':
        ...
        break;
      case '--assistant-name':
        result.assistantName = args[++i] || 'Clawdio';
        break;
    }
  }
...
export async function run(args: string[]): Promise<void> {
  ...
  if (parsed.assistantName !== 'Clawdio') {
    logger.info(
      { from: 'Clawdio', to: parsed.assistantName },
    ...
    for (const mdFile of mdFiles) {
      if (fs.existsSync(mdFile)) {
        ...
        content = content.replace(/^# Clawdio$/m, `# ${parsed.assistantName}`);
