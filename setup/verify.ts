...
export async function run(_args: string[]): Promise<void> {
  ...
  if (mgr === 'launchd') {
    try {
      ...
      if (output.includes('com.codeclaw')) {
        // Check if it has a PID (actually running)
        const line = output.split('\n').find((l) => l.includes('com.codeclaw'));
        if (line) {
    ...
    try {
      execSync(`${prefix} is-active codeclaw`, { stdio: 'ignore' });
      service = 'running';
    } catch {
      try {
        const output = execSync(`${prefix} list-unit-files`, { encoding: 'utf-8' });
        if (output.includes('codeclaw')) {
          service = 'stopped';
        }
      } catch {
  ...
  if (fs.existsSync(path.join(homeDir, '.config', 'codeclaw', 'mount-allowlist.json'))) {
