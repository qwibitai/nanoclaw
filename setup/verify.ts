...
export async function run(_args: string[]): Promise<void> {
  ...
  if (mgr === 'launchd') {
    try {
      ...
      if (output.includes('com.argus')) {
        // Check if it has a PID (actually running)
        const line = output.split('\n').find((l) => l.includes('com.argus'));
        if (line) {
    ...
    try {
      execSync(`${prefix} is-active argus`, { stdio: 'ignore' });
      service = 'running';
    } catch {
      try {
  ...
  if (
    fs.existsSync(
      path.join(homeDir, '.config', 'argus', 'mount-allowlist.json'),
    )
  ) {
