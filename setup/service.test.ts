...
// Helper: generate logrotate config the same way service.ts does
...
describe('logrotate config generation', () => {
  it('includes both log files', () => {
    const config = generateLogrotateConfig('/home/user/nanoclaw');
    expect(config).toContain('/home/user/nanoclaw/logs/nanoclaw.log');
    expect(config).toContain('/home/user/nanoclaw/logs/nanoclaw.error.log');
  });

  it('sets size limit to 50M', () => {
    const config = generateLogrotateConfig('/home/user/nanoclaw');
    expect(config).toContain('size 50M');
  });

  it('keeps 3 rotated files', () => {
    const config = generateLogrotateConfig('/home/user/nanoclaw');
    expect(config).toContain('rotate 3');
  });

  it('uses copytruncate for append-mode files', () => {
    const config = generateLogrotateConfig('/home/user/nanoclaw');
  ...
  it('references issue #1088', () => {
    const config = generateLogrotateConfig('/home/user/nanoclaw');
    expect(config).toContain('#1088');
  });
});
