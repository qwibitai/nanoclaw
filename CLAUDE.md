...
- **`src/container-runner.ts`** -- Mounts `~/.snowflake/perplexity_computer_use.p8` read-only into container, injects `SNOWFLAKE_*` env vars. Removing this breaks Nano's Snowflake access. Also adds `modelOverride` support: injects `ANTHROPIC_MODEL` env var when `ContainerInput.modelOverride` is set (used by task-scheduler to route simple tasks to Haiku).
- **`src/task-scheduler.ts`** -- Sets `modelOverride: 'claude-haiku-4-5-20251001'` for tasks whose id starts with `slack-monitor`, `system-monitor`, or `imessage-summary`. All other tasks use SDK default. If an upstream upgrade overwrites this, those tasks silently revert to Sonnet pricing.
...
- **`container/skills/coo-briefing/`** -- COO Briefing skill: SKILL.md (full spec) + references/property_mapping.json. Triggered by pre-fetch IPC drop (~3:05am) not a fixed schedule. Writes `brief_sent.flag` on completion to prevent duplicate sends. 5:30am fallback task (`coo-brief-fallback-*`) catches pre-fetch failures. property_mapping.json has per-source name fields: `revinate_name`, `str_name`, `alice_name`, `duetto_hotel_code` -- all verified against actual Snowflake data 2026-04-26. Do not overwrite with guessed names.
- **`container/skills/coo-prefetch/`** -- COO Pre-Fetch skill: runs at 3:00am, parallelizes ProfitSword across all 12 hotels with retry pass (48 files: fcst/bud/ly/act), saves all Snowflake (7 queries, STR every day -- str_daily fetches 35 days back for R28 computation) + Toast to disk, then triggers the brief via IPC. Losing it means brief falls back to live fetching and risks timeout.
