...
| `src/channels/`                             | Channel adapter infra (registry, Chat SDK bridge); this install has Telegram enabled and can add more adapters via skills    |
...
NanoClaw keeps channel/provider wiring modular. This local v2 install currently has Telegram enabled in `src/channels/index.ts`; additional adapters/providers can still be installed from long-lived sibling branches by skills:
...
Each `/add-<name>` skill should be idempotent: `git fetch origin <branch>` → copy module(s) into the standard paths → append a self-registration import to the relevant barrel → `pnpm install <pkg>@<pinned-version>` → build.
...
API keys, OAuth tokens, and auth credentials should prefer the OneCLI gateway. Secrets injected through OneCLI are added per request and are not placed in chat context. Some legacy/local workflows still use explicit per-group `envPassThrough` allowlists in `groups/<folder>/container.json`; keep those narrow and move them to OneCLI when practical. See `src/modules/approvals/onecli-approvals.ts`, `ensureAgent()` in `container-runner.ts`, and `containerConfig.envPassThrough` in `src/container-runner.ts`.
...
pnpm run setup:bun                     # Install/check the repo-pinned Bun
...
pnpm run test:container                # Same container tests from repo root
```
...
| Container post-mortems | `data/v2-sessions/<agent-group>/<session>/logs/` — stdout/stderr plus metadata, kill, and exit JSON for each spawned container                                                      |

Containers still run with `--rm`, but the host mirrors stdout/stderr and lifecycle metadata into the session `logs/` directory before Docker removes the container.
...
- **Changing Dockerfile runtime version ARGs** → use exact versions only. CI rejects `ARG *_VERSION=latest`.
