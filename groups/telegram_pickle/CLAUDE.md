...
- Send a compact summary + the web link to chat. Never dump the full plan or full ingredient list in Telegram — the web page at `http://fambots-mac-mini.tail41dff2.ts.net:3100/dashboard#meals` handles that.
...
8. **Send a short summary** to the chat — one line per day (just the meal name), then a link using Telegram's markdown link format so it's tappable: `[Full plan and shopping list](http://fambots-mac-mini.tail41dff2.ts.net:3100/dashboard#meals)` — the hostname has no TLD so Telegram won't auto-link plain URLs. **Never use `&` in chat messages** — it gets HTML-encoded to `&amp;` and breaks Telegram's markdown parser. Use "and" instead.
...
`http://fambots-mac-mini.tail41dff2.ts.net:3100/dashboard#meals`
