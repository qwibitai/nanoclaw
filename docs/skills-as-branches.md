...
This document covers **feature skills** — skills that add capabilities via git branch merges. This is the most complex skill type and the primary way Argus is extended.

Argus has four types of skills overall. See [CONTRIBUTING.md](../CONTRIBUTING.md) for the full taxonomy:
...
This replaces the previous `skills-engine/` system (three-way file merging, `.argus/` state, manifest files, replay, backup/restore) with plain git operations and Claude for conflict resolution.

## How It Works

### Repository structure

The upstream repo (`qwibitai/argus`) maintains:

- `main` — core Argus (no skill code)
...
- `/setup`, `/debug`, `/update-argus`, `/customize`, `/update-skills`
...
- Live in the marketplace repo (`qwibitai/argus-skills`)
...
claude plugin install argus-skills@argus-skills --scope project
```
...
Argus's `.claude/settings.json` registers the official marketplace:
...
```
qwibitai/argus-skills/
  .claude-plugin/
    marketplace.json              # Plugin catalog
  plugins/
    argus-skills/              # Single plugin bundling all official skills
      .claude-plugin/
...
Multiple skills are bundled in one plugin — installing `argus-skills` makes all feature skills available at once. Individual skills don't need separate installation.
...
- Built into `/update-argus` — after merging main, optionally check for skill updates
...
1. Fork `qwibitai/argus` on GitHub (click the Fork button)
2. Clone your fork:
   ```bash
...
Users who previously ran `git clone https://github.com/qwibitai/argus.git` and have local customizations:

1. Fork `qwibitai/argus` on GitHub
2. Reroute remotes:
   ```bash
...
3. From this point, `origin` = their fork, `upstream` = qwibitai/argus
...
In both cases:
- Delete the `.argus/` directory (no longer needed)
...
This is the same as the existing `/update-argus` skill's merge path.

### Updating skills

Run `/update-skills` or let `/update-argus` check after a core update. For each previously-merged skill branch that has new commits, Claude offers to merge the updates.
...
# Create PR from my-fix to qwibitai/argus:main
```
...
4. Add the skill's SKILL.md to the marketplace repo (`qwibitai/argus-skills`)
...
1. Maintains a fork of Argus (e.g., `alice/argus`)
2. Creates `skill/*` branches on their fork with their custom skills
3. Creates a marketplace repo (e.g., `alice/argus-skills`) with a `.claude-plugin/marketplace.json` and plugin structure

### Adding a community marketplace

If the community contributor is trusted, they can open a PR to add their marketplace to Argus's `.claude/settings.json`:
...
Once merged, all Argus users automatically discover the community marketplace alongside the official one.
...
```bash
claude plugin install alice-skills@alice-argus-skills --scope project
```
...
- **Community skills use the same merge pattern.** The SKILL.md just points to a different remote:
  ```bash
  git remote add alice https://github.com/alice/argus.git
  git fetch alice skill/my-cool-feature
  git merge alice/skill/my-cool-feature
  ```
- **Users can also add marketplaces manually.** Even without being listed in settings.json, users can run `/plugin marketplace add alice/argus-skills` to discover skills from any source.
...
A flavor is a curated fork of Argus — a combination of skills, custom changes, and configuration tailored for a specific use case (e.g., "Argus for Sales," "Argus Minimal," "Argus for Developers").
...
AskUserQuestion: "Start with a flavor or default Argus?"
- Default Argus
- Argus for Sales — Gmail + Slack + CRM (maintained by alice)
- Argus Minimal — Telegram-only, lightweight (maintained by bob)

If a flavor is chosen:

```bash
git remote add <flavor-name> https://github.com/alice/argus.git
...
- `upstream` — `qwibitai/argus` (core updates)
...
- **GitHub topics** — flavor forks can tag themselves with `argus-flavor` for searchability
...
Operational skills (`setup`, `debug`, `update-argus`, `customize`, `update-skills`) remain on main in `.claude/skills/`.
...
Before:
```bash
git clone https://github.com/qwibitai/Argus.git
cd Argus
claude
```
...
```
1. Fork qwibitai/argus on GitHub
2. git clone https://github.com/<you>/argus.git
3. cd argus
4. claude
5. /setup
```
...
- Check if `upstream` remote exists; if not, add it: `git remote add upstream https://github.com/qwibitai/argus.git`
- Check if `origin` points to the user's fork (not qwibitai). If it points to qwibitai, guide them through the fork migration.
- **Install marketplace plugin:** `claude plugin install argus-skills@argus-skills --scope project` — makes all feature skills available (hot-loaded, no restart)
...
The `.claude/skills/` directory on `main` retains only operational skills (setup, debug, update-argus, customize, update-skills). Feature skills (add-discord, add-telegram, etc.) live in the marketplace repo, installed via `claude plugin install` during `/setup` or `/customize`.
...
- **Marketplace repo** (`qwibitai/argus-skills`) — single Claude Code plugin bundling SKILL.md files for all feature skills
...
>
> We've simplified how skills work in Argus. Instead of a custom skills engine, skills are now git branches that you merge in.
>
...
> - No more `.argus/` state directory or skills engine
>
...
>
> **If you previously applied skills via the old system**, your code changes are already in your working tree — nothing to redo. You can delete the `.argus/` directory. Future skills and updates use the branch-based approach.
>
...
>
> **Want to run your own skill marketplace?** Maintain skill branches on your fork and create a marketplace repo. Open a PR to add it to Argus's auto-discovered marketplaces — or users can add it manually via `/plugin marketplace add`.
