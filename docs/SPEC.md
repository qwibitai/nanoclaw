...
│  │    • mcp__argus__* (scheduler tools via IPC)                │    │
│  │                                                                │    │
│  └──────────────────────────────────────────────────────────────┘    │
│                                                                       │
└───────────────────────────────────────────────────────────────────────┘
...
│   ├── argus.log               # Host stdout
│   └── argus.error.log         # Host stderr
│   # Note: Per-container logs are in groups/{folder}/logs/container-*.log
│
└── launchd/
    └── com.argus.plist         # macOS service configuration
```
...
export const CONTAINER_IMAGE = process.env.CONTAINER_IMAGE || 'argus-agent:latest';
...
- `{{PROJECT_ROOT}}` - Absolute path to your argus installation
...
Argus uses a hierarchical memory system based on CLAUDE.md files.
...
Argus has a built-in scheduler that runs tasks as full agents in their group's context.
...
Claude: [calls mcp__argus__schedule_task]
        {
...
The `argus` MCP server is created dynamically per agent call with the current group's context.
...
Argus runs as a single macOS launchd service.

### Startup Sequence

When Argus starts, it:
1. **Ensures container runtime is running** - Automatically starts it if needed; kills orphaned Argus containers from previous runs
...
<dict>
    ...
    <string>{{PROJECT_ROOT}}/logs/argus.log</string>
    <key>StandardErrorPath</key>
    <string>{{PROJECT_ROOT}}/logs/argus.error.log</string>
</dict>
</plist>
```
...
```bash
# Install service
cp launchd/com.argus.plist ~/Library/LaunchAgents/
...
|-------|-------|----------|
| No response to messages | Service not running | Check `launchctl list | grep argus` |
| "Claude Code process exited with code 1" | Container runtime failed to start | Check logs; Argus auto-starts container runtime but may fail |
...
- `logs/argus.log` - stdout
- `logs/argus.error.log` - stderr
