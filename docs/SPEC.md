...
│  │    • mcp__sovereign__* (scheduler tools via IPC)                │   │
│  │                                                                │   │
│  └──────────────────────────────────────────────────────────────┘   │
│                                                                      │
└──────────────────────────────────────────────────────────────────────┘
...
│   ├── sovereign.log               # Host stdout
│   └── sovereign.error.log         # Host stderr
│   # Note: Per-container logs are in groups/{folder}/logs/container-*.log
│
└── launchd/
    └── com.sovereign.plist         # macOS service configuration
```
...
export const CONTAINER_IMAGE = process.env.CONTAINER_IMAGE || 'sovereign-agent:latest';
...
- `{{PROJECT_ROOT}}` - Absolute path to your sovereign installation
...
Sovereign uses a hierarchical memory system based on CLAUDE.md files.
...
7. Router invokes Claude Agent SDK:
   ...
   └── mcpServers: sovereign (scheduler)
   │
   ▼
...
Sovereign has a built-in scheduler that runs tasks as full agents in their group's context.
...
Claude: [calls mcp__sovereign__schedule_task]
        {
...
### Sovereign MCP (built-in)

The `sovereign` MCP server is created dynamically per agent call with the current group's context.
...
Sovereign runs as a single macOS launchd service.

### Startup Sequence

When Sovereign starts, it:
1. **Ensures container runtime is running** - Automatically starts it if needed; kills orphaned Sovereign containers from previous runs
...
### Service: com.sovereign

**launchd/com.sovereign.plist:**
```xml
...
<dict>
    ...
    <string>{{PROJECT_ROOT}}/logs/sovereign.log</string>
    <key>StandardErrorPath</key>
    <string>{{PROJECT_ROOT}}/logs/sovereign.error.log</string>
</dict>
</plist>
```
...
```bash
# Install service
cp launchd/com.sovereign.plist ~/Library/LaunchAgents/

# Start service
launchctl load ~/Library/LaunchAgents/com.sovereign.plist

# Stop service
launchctl unload ~/Library/LaunchAgents/com.sovereign.plist
...
|-------|-------|----------|
| No response to messages | Service not running | Check `launchctl list | grep sovereign` |
| "Claude Code process exited with code 1" | Container runtime failed to start | Check logs; Sovereign auto-starts container runtime but may fail |
...
- `logs/sovereign.log` - stdout
- `logs/sovereign.error.log` - stderr
