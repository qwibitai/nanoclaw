...
3. Create a polling scheduler job using `mcp__nanoclaw__scheduler_upsert_job`:
   ```
   ...
   4. Pause this polling job after completion using mcp__nanoclaw__scheduler_pause_job
...
- Verify scheduler job was created: `sqlite3 store/messages.db "SELECT id,name,status,next_run FROM jobs"`
- Check job runs: `sqlite3 store/messages.db "SELECT job_id,status,started_at FROM job_runs ORDER BY started_at DESC LIMIT 20"`
- Ensure job prompt includes proper Parallel MCP tool names
