...
│   ├── tasks/            # Scheduler job commands
│   ├── current_jobs.json     # Read-only: scheduler jobs visible to this group
│   ├── current_job_runs.json # Read-only: scheduler run history visible to this group
...
# Check pending scheduler operations
...
# Check current job runs snapshot
cat data/ipc/{groupFolder}/current_job_runs.json
```

**IPC file types:**
- `messages/*.json` - Agent writes: outgoing WhatsApp messages
- `tasks/*.json` - Agent writes: scheduler operations (upsert/update/delete/pause/resume/trigger, refresh_groups)
- `current_jobs.json` - Host writes: read-only snapshot of scheduler jobs
- `current_job_runs.json` - Host writes: read-only snapshot of recent scheduler runs
