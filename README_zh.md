...
通过 OpenCode，NanoClaw 可以动态重写自身代码，根据您的需求定制功能。
...
```bash
git clone --branch feat/oc-migration https://github.com/wpcapaper/nanoclaw-oc.git
cd nanoclaw-oc
opencode
```

然后运行 `/setup`。OpenCode 会处理一切：依赖安装、身份验证、容器设置、服务配置。

> **注意：** 以 `/` 开头的命令（如 `/setup`、`/add-whatsapp`）是 [OpenCode 技能](https://code.claude.com/docs/en/skills)。请在 `opencode` CLI 提示符中输入，而非在普通终端中。

## 设计哲学

**小巧易懂：** 单一进程，少量源文件。无微服务、无消息队列、无复杂抽象层。让 OpenCode 引导您轻松上手。
...
**为单一用户打造:** 这不是一个框架，是一个完全符合您个人需求的、可工作的软件。您可以 Fork 本项目，然后让 OpenCode 根据您的精确需求进行修改和适配。

**定制即代码修改:** 没有繁杂的配置文件。想要不同的行为？直接修改代码。代码库足够小，这样做是安全的。

**AI 原生:** 无安装向导（由 OpenCode 指导安装）。无需监控仪表盘，直接询问助手即可了解系统状况。无调试工具（描述问题，助手会修复它）。

**技能（Skills）优于功能（Features）:** 贡献者不应该向代码库添加新功能（例如支持 Telegram）。相反，他们应该贡献像 `/add-telegram` 这样的 [OpenCode 技能](https://code.claude.com/docs/en/skills)，这些技能可以改造您的 fork。最终，您得到的是只做您需要事情的整洁代码。

**最好的工具套件，最好的模型:** 本项目运行在 OpenCode SDK 之上，这意味着您直接运行的就是 OpenCode。OpenCode 高度强大，其编码和问题解决能力使其能够修改和扩展 NanoClaw，为每个用户量身定制。
...
没有需要学习的配置文件。直接告诉 OpenCode 您想要什么：
...
如果您想添加 Telegram 支持，不要创建一个 PR 同时添加 Telegram 和 WhatsApp。而是贡献一个技能文件 (`.claude/skills/add-telegram/SKILL.md`)，教 OpenCode 如何改造一个 NanoClaw 安装以使用 Telegram。
...
- `/clear` - 添加一个 `/clear` 命令，用于压缩会话（在同一会话中总结上下文，同时保留关键信息）。这需要研究如何通过 OpenCode SDK 以编程方式触发压缩。
...
- LM Studio、Ollama 或任何兼容 OpenAI API 的 LLM 端点
...
### 2. 安装 oh-my-opencode 插件（必需）

[oh-my-opencode](https://github.com/code-yeongyu/oh-my-opencode) 插件提供了 NanoClaw 依赖的压缩钩子（compaction hooks）和上下文管理功能。没有它，上下文压缩（将长对话总结以保持在 token 限制内）将无法正常工作。

```bash
opencode plugins add oh-my-opencode
```

插件配置已在 `.opencode/oh-my-opencode.jsonc` 中设置好，非必要的钩子已被禁用，只保留 NanoClaw 需要的压缩相关功能。
...
```
渠道 --> SQLite --> 轮询循环 --> 容器 (OpenCode SDK) --> 响应
```
...
我们不希望配置泛滥。每个用户都应该定制它，让代码完全符合他们的需求，而不是去配置一个通用的系统。如果您喜欢用配置文件，告诉助手让它加上。
...
询问助手。"为什么计划任务没有运行？" "最近的日志里有什么？" "为什么这条消息没有得到回应？" 这就是 AI 原生的方法。

**为什么我的安装不成功？**

如果遇到问题，安装过程中助手会尝试动态修复。如果问题仍然存在，运行 `opencode`，然后运行 `/debug`。如果助手发现一个可能影响其他用户的问题，请开一个 PR 来修改 setup SKILL.md。
